...
For every root $\alpha \in \Phi$ there is a ``root subgroup'' morphism $x_{\alpha} \colon R^{(\infty)} \to \St^{(\infty)}(\Phi, R)$ in $\Pro(\Group)$ given by the premorphism $x_\alpha^* = \mathrm{id}_S$, $x_\alpha^{(s)}(a^{(s)}) = x_\alpha^{(s)}(a)$.
...
Let \(\Phi\) be a root system of rank \(\geq 3\) different from \(B_l\) and \(C_l\). Suppose that \(\alpha = \beta + \gamma, \beta, \gamma \in \Phi\) and \(|\alpha| = |\beta| = |\gamma|\). Then there are roots \(\beta_1, \beta_2 \in \Phi \setminus (\mathbb R\beta + \mathbb R\gamma)\) such that \(\beta_1 + \beta_2 = \beta\) and \(|\beta| = |\beta_1| = |\beta_2|\). For any such roots the smallest root subsystem \(\Phi_0 \subseteq \Phi\) closed under addition and containing all of them is isomorphic to \(A_3\) or \(C_3\). There is an embedding \(\Phi_0 \subseteq \mathbb R^3\) such that \(\beta = \mathrm e_1 - \mathrm e_2\), \(\gamma = \mathrm e_2 - \mathrm e_3\), \(\{\beta_1, \beta_2\} = \{\mathrm e_1 - \mathrm e_4, \mathrm e_4 - \mathrm e_2\}\) in the case of \(A_3\), and \(\{\beta_1, \beta_2\} = \{\mathrm e_1 + \mathrm e_3, -\mathrm e_2 - \mathrm e_3\}\) in the case of \(C_3\).
\end{lemma}
\begin{proof}
Note that the set of roots of \(\Phi\) of given length is an irreducible root system \(\Phi'\). In \(\Phi'\) the first claim follows from the fact that the Dynkin diagram is connected and had at least \(3\) vertices. For the second claim, \(\Phi_0\) is irreducible of rank \(3\), so it is either \(A_3\) or \(C_3\) and the roots are short.
\end{proof}

\begin{theorem}\label{SingleRootElimination}
 Let \(R\) be a ring, \(S \subseteq R\) be a multiplicative subset, \(\Phi\) be a root system of rank \(\geq 3\) different from \(B_l\) and \(C_l\), \(\alpha \in \Phi\) be a root. Then \(\St^{(\infty)}(\Phi / \alpha, R) \to \St^{(\infty)}(\Phi, R)\) is an isomorphism of pro-groups.
\end{theorem}
\begin{proof}
 We construct a morphism \(G_\alpha \colon \St^{(\infty)}(\Phi, R) \to \St^{(\infty)}(\Phi / \alpha, R)\) of pro-groups such that \(G_\alpha \circ x_\beta = x_\beta\) for all \(\beta \nparallel \alpha\). For any pair of roots \((\beta, \gamma)\) such that \(|\beta| = |\gamma| = |\alpha|\) and \(\alpha = \beta + \gamma\) let
 \begin{align*}
  x_{\beta, \gamma} \colon R^{(\infty)} \times R^{(\infty)} &\to \St^{(\infty)}(\Phi / \alpha, R),\\
  (b^{(\infty)}, c^{(\infty)}) &\mapsto [x_\beta(b^{(\infty)}), x_\gamma(c^{(\infty)})].
 \end{align*}
 ...
 \begin{align*}
  ...
  &\in \prod_{\substack{i\beta + j\gamma + k\delta \in \Phi\\ k > 0}} x_{i\beta + j\gamma + k\delta}(\ldots),
 \end{align*}
...
By lemma \ref{ThreeRoots} there are roots \(\beta_1, \beta_2 \in \Phi \setminus (\mathbb R \beta + \mathbb R \gamma)\) such that \(|\beta_1| = |\beta_2| = |\beta|\) and \(\beta = \beta_1 + \beta_2\). Moreover, we know all possible arrangements of these roots in a root subsystem of type \(A_3\) or \(C_3\). Then
 \begin{align*}
  x_\beta(\pm b_1^{(\infty)} b_2^{(\infty)})^{x_{\beta, \gamma}(b^{(\infty)}, c^{(\infty)})} &= [x_{\beta_1}(b_1^{(\infty)})^{x_{\beta, \gamma}(b^{(\infty)}, c^{(\infty)})}, x_{\beta_2}(b_2^{(\infty)})^{x_{\beta, \gamma}(b^{(\infty)}, c^{(\infty)})}] =\\
  & [x_{\beta_1}(b_1^{(\infty)}) x_{\alpha + \beta_1}(\ldots), x_{\beta_2}(b_2^{(\infty)}) x_{\alpha + \beta_2}(\ldots)] = x_\beta(\pm b_1^{(\infty)} b_2^{(\infty)}).
 \end{align*}

 Similarly,
 \begin{align*}
  x_{-\beta}(\pm b_1^{(\infty)} b_2^{(\infty)})^{x_{\beta, \gamma}(b^{(\infty)}, c^{(\infty)})} &= [x_{-\beta_1}(b_1^{(\infty)})^{x_{\beta, \gamma}(b^{(\infty)}, c^{(\infty)})}, x_{-\beta_2}(b_2^{(\infty)})^{x_{\beta, \gamma}(b^{(\infty)}, c^{(\infty)})}] =\\
  &= [x_{-\beta_1}(b_1^{(\infty)}) x_{\alpha - \beta_1}(\ldots), x_{-\beta_2}(b_2^{(\infty)}) x_{\alpha - \beta_2}(\ldots)] = x_{-\beta}(\pm b_1^{(\infty)} b_2^{(\infty)}) x_\gamma(\ldots).
 \end{align*}
 ...
 Now suppose that \(\alpha = \beta_1 + \gamma_1 = \beta_2 + \gamma_2\) are two decompositions into a sum of roots with the same length and \(\alpha\), \(\beta_1\), \(\beta_2\) are linearly independent. By lemma \ref{ThreeRoots}, all these roots lie in a common root subsystem of type \(A_3\) or \(C_3\). In the case of \(A_3\) we may assume that \(\alpha = \mathrm e_1 - \mathrm e_4\), \(\beta_1 = \mathrm e_1 - \mathrm e_2\), \(\gamma_1 = \mathrm e_2 - \mathrm e_4\), \(\beta_2 = \mathrm e_1 - \mathrm e_3\), \(\gamma_2 = \mathrm e_3 - \mathrm e_4\). Let \(\delta = \mathrm e_2 - \mathrm e_3\). We have
 \begin{align*}
