...
%(x\otimes y, a, b)\cdot(z\otimes w,c,d)=\\=(xz\otimes yw+xc\otimes y+x\otimes yd+az\otimes w+z\otimes bw+a\otimes d+c\otimes b, ac, bd).
 %\end{multline*}
 $\Rng$ has the structure of a symmetric monoidal category with zero rng as the unit object and $\otimeshat$ as the tensor product. For an rng $R$ we denote by $can$ the natural homomorphism $R^{\otimeshat 2} \to R$ given by $(s \otimes t, u, v) \mapsto st + u + v$.
 ...
 \[ \xymatrix{ \Ker(\varepsilon) \ar[r]^(0.45){\Delta} & \Ker(\varepsilon) ^{\otimeshat 2} \ar[r]^(0.47){g \otimeshat h} & R^{(s)} \otimeshat R^{(s)} \ar[r]^(0.6){can} & R^{(s).}} \]
\end{rem}
...
\begin{equation}\label{eq:house} \xymatrix@R=48pt@C=72pt@!0{
 & Y \ar[dr] \ar[dl]_(0.4){\Delta} & \\
Y \otimeshat Y \ar[d] &  & \frac R{s_1} \otimes K \ar[d]^{s_1s_2 \otimes \Delta} \\
(\frac R{s_1} \otimes K)^{\otimeshat 2} \ar[rr]^{u} \ar[d]^{(\mathrm{id} \otimes h_1) \otimeshat (\mathrm{id} \otimes h_2)} && \frac R{s_1^2 s_2} \otimes K^{\otimeshat 2} \ar[d]^{\mathrm{id} \otimes (h_1 \otimeshat h_2)} \\
(R^{(ss_1s_2)})^{\otimeshat 2} \ar[d]^{can} && \frac R{s_1^2 s_2} \otimes (R^{(ss_1^2 s_2)})^{\otimeshat 2} \ar[d]^{\mathrm{id}\otimes can} \\
R^{(ss_1s_2)} \ar[r] & R^{(s)} & \frac R{s_1^2 s_2} \otimes R^{(ss_1^2 s_2).} \ar[l]
}\end{equation}
The homomorphism $u$ in the above diagram is induced by canonical homomorphisms $\tfrac R {s_1} \to \tfrac R {s_1^2s_2}\text{ and }\tfrac{R} {s_1} \otimes \tfrac R{s_1} \to \tfrac R{s_1^2s_2}.$
...
\begin{proof}
 ...
 & Y \ar[d] \ar@{^{(}->}[ul] \ar[r]^(0.4){\{b_i\}} & \tfrac R {s_1} \otimes K \ar[u] \ar[r]^(0.42){s_1s_2\otimes\mathrm{Cot}_\beta} & \tfrac R {s_1^2s_2} \otimes X\ZZ[X] \ar[ru]\\ 
 & Y \otimeshat Y  \ar@{^{(}->}[dl] \ar@{-->}[rr] & & (\tfrac R {s_1} \otimes X\ZZ[X])^{\otimeshat 2} \ar[r] \ar[u]^{u} & (S^{-1}R \otimes X\ZZ[X])^{\otimeshat 2} \ar[uu]^{can} \\
 ...
 Following the procedure described in~\eqref{eq:bis} we can choose $s_1$ and $\{b_i\}$ in such a manner that the top left square of the above diagram commutes. Also, there is a dashed arrow making the bottom pentagon commute. Finally, there exists sufficiently large $s_2$ such that the central square of the diagram commutes. This commutativity implies~\eqref{eq:equivariance-statement} (the top path gives the right-hand side while the bottom one gives the left-hand side; the homomorphism $u$ is constructed similarly to~\eqref{eq:house}). Thus, by \cref{SteinbergPresentation} the morphism \(\mathrm{st} \colon \St^{(\infty)}(\Phi, S^{-1} R) \to \GG^{(\infty)}(\Phi, R)\) is \(\St(\Phi, S^{-1} R)\)-equivariant.
\end{proof}
