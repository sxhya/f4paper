...
Clearly, the resulting morphism of pro-groups is independent of the choice of \(s_1\). Hence it is also independent of the choice of \(s_0\), \(a_i\), and \(\up g{a_i}\). For example, if \(\up g{a_i} + \delta_i\) are another choices of \(\up g{a_i}\) and the other parameters are fixed, then \((s_2 \otimes 1) \delta_i = 0\) for some \(s_2 \in S\) and \((\mathrm{id} \otimes h)(\delta_i) = 0 \in \frac R{s_1 s_2} \otimes_R R^{(s s_1 s_2)}\). It is not hard to check that $\up 1{(-)}$ is the identity automorphism of $\mathrm{id}_{\GG^{(\infty)}(\Phi, R)}$ and that for all \(g, g' \in \GG_{\mathrm{sc}}(\Phi, S^{-1} R)\) one has $\up{gg'}{(-)} = \up{g}{(\up{g'}{(-)})}$.
