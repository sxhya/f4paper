...
%(x\otimes y, a, b)\cdot(z\otimes w,c,d)=\\=(xz\otimes yw+xc\otimes y+x\otimes yd+az\otimes w+z\otimes bw+a\otimes d+c\otimes b, ac, bd).
 ...
 Notice that if $R_1$ and $R_2$ are rngs then their $\ZZ$-module tensor product \(R_1 \otimes_{\mathbb Z} R_2\) has the natural structure of an rng. Obviously, $R_1 \otimes R_2 \subseteq R_1 \otimeshat R_2$. A priori there are no homomorphisms $R_i \to R_1 \otimes R_2$. However, if $R_1$ happens to be a ring then there is a canonical homomorphism $R_2 \to R_1 \otimes R_2$.
...
Notice that the tuple $(s_0, s_1, \{b_i\})$ with the above properties is not unique.
...
Let $h$ be an element of $\Rng(\Ker(\varepsilon), R^{(ss_1)})$.
Notice that the images of $b_i$ under $\mathrm{id}\otimes h$ satisfy all $f_j$'s.
Now the element $\theta_s(h)$ now can be constructed as the composite of $\mathrm{Coconj}$ and the unique rng homomorphism $T \to R^{(s)}$ making the following diagram commute:
\[ \xymatrix{ & \{ a_i\} \ar@{^{(}->}[d] \ar[r]^(0.4){\{b_i\}} & \tfrac R {s_1} \otimes \Ker(\varepsilon) \ar[d]^{\mathrm{id} \otimes h} \\
             \Ker(\varepsilon) \ar[r]^(0.57){\mathrm{Coconj}} \ar@/_1pc/[rr]_{\theta_s(h)} & T \ar@{-->}[r] & \tfrac R {s_1} \otimes_R R^{(ss_1)} \ar@{=}[r] & R^{(s)}.   } \]

It is clear that the morphism~\eqref{eq:conj-g} defined by $\theta$ is independent of the choices of $s_0$ and $s_1$. Consequenty, it is also independent of the choices of $\{a_i\}$ and $\{b_i\}$. Indeed, if the images of $\{b_i\}, \{b_i'\}$ in $S^{-1}R \otimes \Ker(\varepsilon)$ coincide, then there exists $s_2\in S$ such that all the differences $b_i - b_i'$ are annihilated by $s_2 \otimes \mathrm{id} \colon \tfrac{R} {s_1} \otimes \Ker(\varepsilon) \to \tfrac R {s_1s_2} \otimes \Ker(\varepsilon)$.

We need to check that \(\up g{(-)}\) is a morphism of pro-groups.
Recall from~\cref{sec:group-objects} that it is enough to check that it is morphism of group objects in \(\Pro(\Set)\), which easily follows from the following commutative diagram (in which \(s_1\) is sufficiently large):
\[\xymatrix@R=48pt@C=72pt@!0{
&
\mkern-36mu S^{-1} R \otimes \Ker(\varepsilon) \otimeshat \Ker(\varepsilon) &&
S^{-1} R \otimes \Ker(\varepsilon) \otimeshat S^{-1} R \otimes \Ker(\varepsilon) \ar[ll] \mkern-18mu & \\
T \ar[dr]_(0.4){\substack{a_i \mapsto\\ (\mathrm{id} \otimes h_1 h_2)(b_i)} \mkern-18mu} \ar[ur]^(0.4){g \otimes \Delta} &
...
T \otimeshat T \ar[dl]^(0.3){\mkern-18mu \substack{a_i \otimes a_j \mapsto\\ (\mathrm{id} \otimes h_1 \otimeshat \mathrm{id} \otimes h_2)(b_i \otimes b_j)}} \ar[ul]_(0.4){\quad g \otimes \mathrm{id} \otimeshat g \otimes \mathrm{id}} \\
&
\frac R{s_1^2} \otimes_R R^{(ss_1^2)} &&
