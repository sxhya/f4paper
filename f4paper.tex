...
\begin{proof}
 ...
 The next step of the proof is to verify that the constructed action satisfies Steinberg relations~\eqref{R2}--\eqref{R4} defining the group $\St(\Phi, S^{-1}R)$.
 Let $R_{\alpha, \beta}(a, b)$, $a, b\in S^{-1}R$ be one of these relations (interpreted as an element of the free group on $x_\alpha(a)$). Denote by $\Sigma \subseteq \Phi$ be the special subset consisting of all roots in the cone spanned by $-\alpha$ and $-\beta$, i.e. $\Sigma = \Phi \cap (\ZZ_{\leq 0} \alpha + \ZZ_{\leq 0} \beta)$. Let $\gamma \in \Phi \setminus \Sigma$ be a root. We claim that an argument similar to the proof of~\cref{endomor-elim} shows that the result of conjugation of $x_\gamma$ with $R_{\alpha, \beta}(a, b)$ coincides with $x_\gamma$. Indeed, during each consecutive application of the conjugation action from~\cref{root-action} no root subgroup morphism $x_\delta$, $\delta \in \Sigma$ may appear, which guarantees that the computation does not ``stuck''. By~\cref{DoubleRootElimination} this implies that the automorphism of conjugation with $R_{\alpha,\beta}(a, b)$ coincides with the trivial automorphism of $\St^{(\infty)}(\Phi, R)$.
