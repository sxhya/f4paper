...
\[ [x_\alpha(a),\ x_\beta(b)] = \prod\limits_{\substack{i\alpha + j\beta\in\Phi \\ i,j >0}} x_{i\alpha + j\beta}(N_{\alpha \beta}^{i j} \cdot a^i b^j), \]
where $N^{1,2}_{\alpha,\beta} := -N^{2,1}_{\beta, \alpha}$ and $N^{1,1}_{\alpha,\beta}:=N_{\alpha,\beta}$.
