...
To obtain a morphism $f \colon \St^{(\infty)}(\Phi, R) \to G^{(\infty)}$ it suffices to construct a collection of pro-group morphisms \(f_{\alpha} \colon R^{(\infty)} \to G^{(\infty)}\), $\alpha\in\Phi$, satisfying the pro-analogues of~\eqref{R2}--\eqref{R4}, in which $x_\alpha$'s are replaced with $f_{\alpha}$'s.
\end{lemma}
\begin{proof}
...
\begin{itemize}
  \item Case \(\Phi_0 \cong \rA_2\). The claim follows from~\cref{ThreeRoots}.
  \item Case $\Phi_0 \cong \rB_2$ (only possible for $\Phi=\rF_4$). Since all root subsystems of type $\rB_2$ lie in the same orbit under the action of $W(\rF_4)$ (cf. e.\,g.~\cite[Table~8]{Ca72}), we may assume that $\Phi_0$ corresponds to the central edge of the Dynkin diagram of $\rF_4$ and $\delta$ is one of the nodes incident to it, in which case the claim is obvious.
  \item Case $\Phi_0 \cong \rA_1+\rA_1$. If $\Phi$ is simply-laced or $\Phi_0$ has roots of different length, acting by a suitable element of $W(\Phi)$ we may achieve that the basis vectors of $\Phi_0$ correspond to a pair of unjoined nodes of the Dynkin diagram of $\Phi$, in which case the claim is also obvious. Finally, if $\Phi=\rF_4$ and the roots of $\Phi_0$ have the same length, our claim follows from the fact that it holds already for the root subsystem $\Phi'\cong \rD_4$ consisting of roots of $\Phi$ having the same length as the roots of $\Phi_0$.
\end{itemize}
  It is clear that
...
\begin{rem} \label{rem:xpma-presentation}
 One can obtain the analogue of~\cref{SteinbergPresentation} for the pro-group $\St^{(\infty)}(\Phi\setminus\{\alpha\}, R)$ by repeating the same proof verbatim. In particular, $x_\beta$ for $\beta\neq \alpha$ generate $\St^{(\infty)}(\Phi\setminus\{\alpha\}, R)$ in the categorical sense.
\end{rem}
...
Since $\alpha+\beta_1$ and $\beta$ form a sharp angle, the latter commutator is trivial by~\eqref{eq:comm-mult-rhs} and~\eqref{R2-bg}.  
...
= x_{\beta_2, \gamma_2}(b^{(\infty)} c^{(\infty)}, d^{(\infty)}). & \end{align*}
 ...
 Thus, in both cases we obtain the fact that
 \begin{equation}\label{eq:balance}
 x_{\beta_1, \gamma_1}(b^{(\infty)}, c^{(\infty)} d^{(\infty)}) = x_{\beta_2, \gamma_2}(b^{(\infty)} c^{(\infty)}, d^{(\infty)}).
 \end{equation}
 It follows from~\eqref{eq:balance} that $x_{\beta_1,\gamma_1}(b^{(\infty)}, u_1^{(\infty)}u_2^{(\infty)}d^{(\infty)}) = x_{\beta_1,\gamma_1}(b^{(\infty)}u_1^{(\infty)}u_2^{(\infty)},d^{(\infty)})$ therefore from~\cref{RingGeneration} we obtain the fact that
...
First, let us consider the case \(\alpha = \beta + \gamma\), in which \(\alpha\), \(\beta\) are short and \(\gamma\) is long. Consider a decomposition \(\beta = \beta_1 + \beta_2\) for some short roots \(\beta_i\). The smallest root subsystem containing \(\beta_i\) and \(\gamma\) is of type \(\rC_3\), so we may assume that \(\alpha = \mathrm e_1 + \mathrm e_3\), \(\beta = \mathrm e_1 - \mathrm e_3\), \(\beta_1 = \mathrm e_1 - \mathrm e_2\), \(\beta_2 = \mathrm e_2 - \mathrm e_3\), \(\gamma = 2\mathrm e_3\). Set $\varepsilon_1 = N_{\beta_1, \beta_2},$ $\varepsilon_2 = N_{\beta_2,\gamma}^{2,1},$ $\varepsilon_3 = N_{\beta_2, \gamma},$ $\varepsilon_4 = N_{\beta_1, 2\beta_2+\gamma}^{2,1},$ $\varepsilon_5 = N_{\beta_1, 2\beta_2+\gamma},$ $\varepsilon_6 = N_{\beta_1,\beta_2+\gamma},$ $\varepsilon_7 = N_{\beta_2+\gamma, \beta}.$ From~\eqref{eq:comm-new} and \eqref{R4} we immediately obtain
 \begin{multline*}
  ...
  =[x_{\beta_1}(b_1^{(\infty)}),\ x_{2\beta_2+\gamma}(-\varepsilon_2{b_2^{(\infty)}}^2c^{(\infty)}) x_{\beta_2+\gamma}(-\varepsilon_3b_2^{(\infty)}c^{(\infty)}) x_{\beta_2}(b_2^{(\infty)})] x_{\beta}(-\varepsilon_1b_1^{(\infty)}b_2^{(\infty)}). \end{multline*}
 Using~\eqref{eq:comm-mult-rhs}, \cref{cor:elim-lhs} and relations~\eqref{R2}--\eqref{R4}, we can rewrite the latter expression as follows:
 \begin{multline*}
 \bigl[x_{\beta_1}(b_1^{(\infty)}), x_{2\beta_2+\gamma}(-\varepsilon_2{b_2^{(\infty)}}^2c^{(\infty)})\bigr] \cdot \up{x_{2\beta_2+\gamma}(-\varepsilon_2{b_2^{(\infty)}}^2c^{(\infty)})}{\bigl[x_{\beta_1}(b_1^{(\infty)}), x_{\beta_2+\gamma}(-\varepsilon_3b_2^{(\infty)}c^{(\infty)})\bigr]} \cdot\\
 \cdot \bigl[x_{2\beta_2+\gamma}(-\varepsilon_2{b_2^{(\infty)}}^2c^{(\infty)}) x_{\beta_2+\gamma}(-\varepsilon_3b_2^{(\infty)}c^{(\infty)}), x_{\beta}(\varepsilon_1b_1^{(\infty)}b_2^{(\infty)})\bigr]\\
 = x_{\beta_1 + 2\beta_2 + \gamma}(-\varepsilon_2 \varepsilon_5 b_1^{(\infty)} {b_2^{(\infty)}}^2 c^{(\infty)})\, x_{2\beta + \gamma}(-\varepsilon_2 \varepsilon_4 {b_1^{(\infty)}}^2 {b_2^{(\infty)}}^2 c^{(\infty)})\\
 \cdot \widetilde x_\alpha(-\varepsilon_3 \varepsilon_6 b_1^{(\infty)} b_2^{(\infty)}c^{(\infty)})\,
 x_{\beta_1 + 2\beta_2 + \gamma}(-\varepsilon_1 \varepsilon_3 \varepsilon_7 b_1^{(\infty)} {b_2^{(\infty)}}^2 c^{(\infty)})\\
 = x_{2\beta + \gamma}( - \varepsilon_2 \varepsilon_4 {b_1^{(\infty)}}^2{b_2^{(\infty)}}^2c^{(\infty)}) x_{\beta_1 + 2\beta_2 + \gamma}(-(\varepsilon_2\varepsilon_5 +\varepsilon_1\varepsilon_3\varepsilon_7) b_1^{(\infty)}{b_2^{(\infty)}}^2c^{(\infty)}) \\
 \cdot \widetilde{x}_{\alpha}(-\varepsilon_3 \varepsilon_6 b_1^{(\infty)}b_2^{(\infty)}c^{(\infty)}).
\end{multline*}
Substituting the triples $(\beta_1, \beta_2+\gamma, \beta_2)$, $(\beta_1, \beta_2, \gamma)$, $(\beta_1, \beta_1+\beta_2, \beta_2+\gamma)$ into~\eqref{eq:cocycle2} we obtain the equalities \[\varepsilon_2 \varepsilon_3 \varepsilon_5 = - \varepsilon_1 \varepsilon_7,\ \varepsilon_3 \varepsilon_6  = N_{\beta, \gamma}\varepsilon_1,\ 2 \varepsilon_4 \varepsilon_5 \varepsilon_7 = - \varepsilon_6 N_{\beta,\alpha},\]
which together with~\eqref{eq:N21def} imply \[\varepsilon_2 \varepsilon_4 = \tfrac{1}{2} \varepsilon_1 \varepsilon_3 \varepsilon_5 \varepsilon_7 \cdot \varepsilon_5 \varepsilon_6 \varepsilon_7 N_{\beta,\alpha}= \tfrac{1}{2} \varepsilon_1 \varepsilon_3 \varepsilon_6 N_{\beta, \alpha} = \tfrac{1}{2} N_{\beta,\gamma} N_{\beta,\alpha}= N_{\beta, \gamma}^{2, 1}.\] Thus, using~\cref{RingGeneration} we conclude that \[[x_\gamma(c^{(\infty)}), x_\beta(b^{(\infty)})] =
 \widetilde{x}_\alpha(-N_{\beta, \gamma} b^{(\infty)} c^{(\infty)}) x_{2\beta+\gamma}(-N_{\beta,\gamma}^{2,1} {b^{(\infty)}}^2 {c^{(\infty)}}),\] which is equivalent to~\eqref{eq:elim-rhs-r4-1}.
 ...
 Substituting the triples $(\beta, \gamma_2, \gamma_1)$, $(\beta, \beta+\gamma_2, \gamma_1)$ into~\eqref{eq:cocycle2} we obtain the identities $\varepsilon_3 \varepsilon_5 = \varepsilon_1 N_{\beta,\gamma}$,
$2\varepsilon_2\varepsilon_3 \varepsilon_4 = \varepsilon_5N_{\beta,\beta+\gamma}$ (together with~\eqref{eq:N21def} they also imply that $\varepsilon_2\varepsilon_4 = \tfrac{1}{2} \varepsilon_3 \varepsilon_5 N_{\beta,\beta+\gamma} = \varepsilon_1 N_{\beta,\gamma}^{2,1}$). Thus, the above equality together with~\cref{RingGeneration} imply~\eqref{eq:elim-rhs-r4-1}. \end{proof}
...
\begin{lemma}\label{omitted-relation}
In the definition of $\St(\Phi \setminus \{\alpha\}, R^{(s)})$ we may omit all relations of type
\[[x_\beta(b), x_\gamma(c)] = 1\]
for long roots \(\beta, \gamma\) such that \(\beta + \gamma = 2\alpha\), so the resulting pro-group is isomorphic to \(\St^{(\infty)}(\Phi \setminus \{\alpha\}, R)\).
\end{lemma}
\begin{proof}
Note that \(\Phi\) is of type \(\mathsf F_4\), otherwise there are no such relations. Without loss of generality, \(\alpha = \frac{\mathrm e_1 + \mathrm e_2 + \mathrm e_3 + \mathrm e_4}2\), \(\beta = \mathrm e_1 + \mathrm e_2\), and \(\gamma = \mathrm e_3 + \mathrm e_4\). Let \(\gamma_1 = \mathrm e_4\) and \(\gamma_2 = \mathrm e_3 - \mathrm e_4\), then
\begin{align*}
\bigl[x_\beta(b^{(\infty)}), x_\gamma(N_{\gamma_1, \gamma_2}^{2, 1} {c^{(\infty)}}^2 d^{(\infty)})\bigr] &= \bigl[x_\beta(b^{(\infty)}), x_{\gamma_1 + \gamma_2}(N_{\gamma_1, \gamma_2} c^{(\infty)} d^{(\infty)})\, x_\gamma(N_{\gamma_1, \gamma_2}^{2, 1} {c^{(\infty)}}^2 d^{(\infty)})\bigr]\\
&= \bigl[x_\beta(b^{(\infty)}), \bigl[x_{\gamma_1}(c^{(\infty)}), x_{\gamma_2}(d^{(\infty)})\bigr]\bigr] = 1,
\end{align*}
since \(\beta + \gamma_1, \beta + \gamma_2 \notin \Phi \cup \{0, 2\alpha\}\). By the same argument as in the proof of \cref{RingGeneration}, \({c^{(\infty)}}^2 d^{(\infty)} \colon R^{(\infty)} \times R^{(\infty)} \to R^{(\infty)}\) is a split epimorphism of pro-sets. Hence \([x_\beta(b^{(\infty)}), x_\gamma(c^{(\infty)})] = 1\).
\end{proof}
