...
We also set  $S = R \setminus M$ and denote by \[\pi_R \colon R^{(\infty)} \to R,\ \pi_{\St} \colon \St^{(\infty)}(\Phi, R) \to \St(\Phi, R)\text{ and } \pi_{\GG} \colon \GG^{(\infty)}(\Phi, R) \to \GG(\Phi, R).\] the canonical pro-group morphisms.
...
\begin{proof}
 ...
 By~\cref{ChevalleyLocalAction}, the element \(g\) acts trivially on the pro-group \(\St^{(\infty)}(\Phi, R)\). Consequently, we obtain that 
 \begin{multline*}
...
\end{lemma}
\begin{proof}
 Let $g$ be an element of $\GG(\Phi, R)\) and \(\alpha \in \Phi\) be a root. Consider the ideal
 \[I = \{a \in R \mid \up g{t_\alpha(Ra)} \in \mathrm E(\Phi, R)\}.\]
 Again, we need to show that \(I = R\). Let \(M \trianglelefteq R\) be a maximal ideal. By~\cref{ChevalleyLocalAction} \(g\) acts on the pro-group \(\St^{(\infty)}(\Phi, R)\) in such a way that \(\mathrm{st} \colon \St^{(\infty)}(\Phi, R) \to \GG^{(\infty)}(\Phi, R)\) is equivariant. Consequently, we obtain that
 \[
...
\end{theorem}
\begin{proof}
 We already know from~\cref{CentralityK2} that the Steinberg group \(\St(\Phi, R)\) is a central extension of the elementary group \(\mathrm E(\Phi, R)\). For any \(g \in \GG_{sc}(\Phi, R)\) it remains to construct an automorphism \(\up g{(-)} \colon \St(\Phi, R) \to \St(\Phi, R)\) such that \(\mathrm{st}\) is equivariant. Such an endomorphism is necessarily unique and multiplicative in \(g\).
 
 Let $g$ be an element of $\GG_{sc}(\Phi, R)\) and $\alpha$ be a root of $\Phi$. Since \(\mathrm E(\Phi, R) \trianglelefteq \GG_{sc}(\Phi, R)\) by~\cref{Normality}, the set
 $Y_\alpha(a) = \mathrm{st}^{-1}\bigl(\up g{t_\alpha(a)}\bigr)$
 is nonempty for all \(a \in R\). Moreover, it is a coset of \(\mathrm K_2(\Phi, R)\) and \(Y_\alpha(a + a') = Y_\alpha(a) Y_\alpha(a')\).

 First of all, let us show that \([Y_\alpha(a), Y_\beta(b)] = 1\) in the case when \(\alpha + \beta \notin \Phi \cup \{0\}\). For any such \(\alpha, \beta\) and any \(a \in R\) let $h$ be an element of $Y_\alpha(a)\). Consider the ideal
 \[I = \{b \in R \mid [h, Y_\beta(bR)] = 1\}.\]
 If \(M \trianglelefteq R\) is a maximal ideal, then
 \[[h, \pi_{\St}(\up g {x_\beta(b^{(\infty)})})] = \pi_{\St}(\up{g t_\alpha(a)} {x_\beta(-b^{(\infty)})}) \pi_{\St}(\up g {x_\beta(-b^{(\infty)})}) = 1.\]
 Thus, \(I \not \subseteq M\) and consequently \(I = R\) and we are done.

 Now let $\alpha$ be a root of $\Phi$. There exist roots \(\beta, \gamma \in \Phi\) of the same length such that \(\alpha = \beta + \gamma\). Denote by \(y_\alpha(a)\) the only element of \([Y_\beta(N_{\beta, \gamma}^{-1}), Y_\gamma(a)]\). Clearly, \(y_\alpha \colon R \to \St(\Phi, R)\) is a homomorphism. For any maximal ideal \(M \trianglelefteq R\) we have
 \begin{align*}
 ...
 \end{align*}
 For any maximal ideal \(M \trianglelefteq R\) we have
 \begin{align*}
 ...
 \end{align*}
 It follows that \(J \not \leq M\), i.e. \(J = R\). The verification of~\eqref{R3} is similar but easier.
\end{proof}
