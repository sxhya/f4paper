...
It is easy to check that the homomorphism $ T \to \frac R{s_1} \otimes_R R^{(s s_1)} \cong R^{(s)}$ defined by $a_i \mapsto (\mathrm{id} \otimes h)(\up g{a_i})$
...
Clearly, the resulting morphism of pro-groups is independent on the choices of \(s_0\), \(s_1\), and \(a_i\). It is not hard to check that $\up 1{(-)} = \mathrm{id}_{\GG^{(\infty)}(\Phi, R)}$ and that for all \(g, g' \in \GG_{\mathrm{sc}}(\Phi, S^{-1} R)\) one has $\up{gg'}{(-)} = \up{g}{(\up{g'}{(-)})}$.
