...
\[\mathrm{Coconj} \colon K \to H_\Phi \otimes \Ker(\varepsilon),\ h \mapsto \sum h_{(1)} \varsigma(h_{(3)}) \otimes h_{(2)}.\]
We denote by $T$ the image of $K$ under $\mathrm{Coconj}$ and by $a_i$ the images of the elements $y_i \in K$.
It is clear that $\mathrm{Coconj}$ is injective, so $T$ is isomorphic to $\Ker(\varepsilon)$.
