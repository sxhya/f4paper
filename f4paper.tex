...
\begin{df}\label{def:Steinberg-homotope}
 Let $R$ be a ring and $S$ be a multiplicative subset of $R$. Also let $\Phi$ be a root system of rank $\geq 3$. Applying the Steinberg group functor $\St(\Phi, -)$ to the projective system of rngs $R^{(s)}$ (see~\cref{ring-homotope}) we obtain a projective system of groups, whose formal projective limit will be denoted by $\St^{(\infty)}(\Phi, R)$ and will be called the {\it Steinberg pro-group}. 
\end{df}
...
\begin{rem}
 The definition of the group $\GG(\Phi, R^{(s)})$ can be reformulated in terms of Hopf algebras. Denote by \(H_\Phi\) be the Hopf \(\ZZ\)-algebra corresponding to the group scheme \(\GG_{\mathrm{sc}}(\Phi, -)\). Thus, $\GG_\mathrm{sc}(\Phi, R) = \mathbf{Ring}(H_\Phi, R)$.
...
\begin{proof}  
 ...
 Since $\mathrm{st}(H)$ is contained in the congruence subgroup $\GG_\mathrm{sc}(\Phi, R, I)$ we obtain from the standard commutator formula~\cite[Theorem~1]{Va86} that $\mathrm{st}(H')\subseteq\E_\mathrm{sc}(\Phi, R, I)$. Thus, $H'\subseteq \St(\Phi, R, I) \cdot \mathrm{K}_2(\Phi, R)$. Finally, from our assumptions we obtain that
