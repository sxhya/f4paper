...
Several different methods of computing signs of the structure constants have been described in the literature, see e.\,g.~\cite{VP}. In any case, these constants must satisfy certain relations, which suffice for the purposes of the present article. 
...
\begin{df}\label{def:Steinberg-homotope}
 Let $R$ be an ring with and $S$ be a multiplicative subset of $R$. Also let $\Phi$ be a root system of rank $\geq 3$. Applying the Steinberg group functor $\St(\Phi, -)$ to the projective system of rngs $R^{(s)}$ (see~\cref{ring-homotope}) we obtain a projective system of groups, whose formal projective limit will be denoted by $\St^{(\infty)}(\Phi, R)$ and will be called the {\it Steinberg pro-group}.
...
\begin{df}
 ...
 $\GG(\Phi, R^{(s)} \rtimes R, R^{(s)}) = \Ker\left(\GG_{sc}(\Phi, R^{(s)} \rtimes R) \to \GG_{sc}(\Phi, R)\right)$ with the structure morphisms induced by the structure morphisms of \(R^{(\infty)}\). Define the {\it simply-connected Chevalley pro-group} $\GG^{(\infty)}(\Phi, R)$ as the formal projective limit of this system. 
 
 The definition may be formulated in terms of Hopf algebras. Let \(H\) be the Hopf \(\ZZ\)-algebra corresponding to \(\GG_{sc}(\Phi, -)\) and \(\varepsilon \colon H \to \ZZ\) be the augmentation map. Then \(\GG^{(s)}(\Phi, R) \cong \Rng(\Ker(\varepsilon), R^{(s)})\). This isomorphism commutes with the structure morphisms of the pro-objects.
\end{df}
...
Similarly, \(\GG_{sc}(\Phi, S^{-1} R)\) acts on the Chevalley pro-group \(\GG^{(\infty)}(\Phi, R)\) by automorphisms. Indeed, denote by \(H\) the Hopf \(\ZZ\)-algebra of \(\GG_{sc}(\Phi, -)\) with the structure of a Hopf algebra given by the triple $(\Delta, S, \varepsilon)$. Recall that \(H\) is a finitely presented commutative \(\ZZ\)-algebra. Denote by ${}_RH$ the Hopf algebra $H\otimes_\ZZ R$. Take \(g \in \GG_{sc}(\Phi, S^{-1} R) \cong \mathbf{Ring}_\ZZ(H, S^{-1}R) \cong \mathbf{Ring}_R({}_RH, S^{-1} R)\) and a sufficiently large \(s \in S\). For any \(h \in \GG^{(ss')}(\Phi, R) \cong \Rng_R(\Ker(\varepsilon), R^{(ss')})\) denote by \(\up gh\) the composition
\[\Ker(\varepsilon) \xrightarrow{(\mathrm{id} \otimes \mathrm{id} \otimes S) \circ \Delta^2} H \otimes \Ker(\varepsilon) \otimes H \xrightarrow{g \otimes h \otimes g} S^{-1} R \otimes R^{(ss')} \otimes S^{-1} R \to R^{(s')}.\]
Here the last map is only partially defined. But we may choose \(s\) such that the composition is defined on a finite generating set of \(\Ker(\varepsilon)\) and maps finitely many relations between them to \(0\). It is easy to see that we get a homomorphism \(\GG_{sc}(\Phi, S^{-1} R) \to \mathrm{Aut}(\GG^{(\infty)}(\Phi, R))\)
