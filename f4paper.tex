...
The commutativity of the whole diagram implies that the images of $\theta_{ss_1s_2}(h_1h_2)$ $\theta_{ss_1s_2}(h_1) \theta_{ss_1s_2}(h_2)$ in $\GG_\mathrm{sc}(\Phi, R^{(s)})$ coincide. Consequently, the morphism~\eqref{eq:conj-g} is a morphism of group of objects in $\Pro(\Set)$ and hence a morphism of pro-groups (cf.~\cref{sec:group-objects}).
