...
\begin{df}\label{root-action}
 ...
 %For every root \(\beta \in \Phi\) and every \(g\) in the standard torus \(\Torus(\Phi, S^{-1} R)\) of \(\GG_{sc}(\Phi, S^{-1} R)\) let  \[\up g{x_\beta(b^{(\infty)})} = x_\beta(\beta(g) b^{(\infty)}).\]
 %For every nonparallel \(\alpha, \beta \in \Phi\) and \(u \in S^{-1} R\) set
 \begin{align*}
...
\end{prop}
\begin{proof}
 %In is clear from the definitions that \(\Torus(\Phi, S^{-1} R)\) acts on \(\St^{(\infty)}(\Phi, R)\).
 ...
 %Moreover, the actions of \(g x_\alpha(u) g^{-1}\) and \(x_\alpha(\alpha(g) u)\) coincide for all \(g \in \Torus(\Phi, S^{-1} R)\), \(u \in R\).
...
\end{theorem}
\begin{proof}
 %We already know from~\cref{CentralityK2} that the Steinberg group \(\St(\Phi, R)\) is a central extension of the elementary group \(\mathrm E(\Phi, R)\).
 Let $g$ be an element of $\GG_{sc}(\Phi, R)$. Our first goal is to construct an automorphism \(\up g{(-)} \colon \St(\Phi, R) \to \St(\Phi, R)\).% such that \(\mathrm{st}\) is equivariant. 
 %Such an endomorphism is necessarily unique and multiplicative in \(g\).
 ...
 Now let $\alpha$ be a root of $\Phi$. By our assumptions on $\Phi$ there exist roots \(\beta, \gamma \in \Phi\) of the same length such that \(\alpha = \beta + \gamma\). Denote by \(y_\alpha(a)\) the only element of the set \([Y_\beta(N_{\beta, \gamma}), Y_\gamma(a)]\). Clearly, \(y_\alpha \colon R \to \St(\Phi, R)\) is a homomorphism. 
 ...
 \end{align*}
 Now from~\eqref{eq:scm1} we obtain that
 \begin{align*}
