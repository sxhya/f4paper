...
On the other hand, the analogue of Suslin's normality theorem for Chevalley groups of rank $\geq 2$ and an arbitrary commutative ring $R$ was soon proved by G.~Taddei, see~\cite{Ta86}. 

The first progress since~\cite{vdK} was the result of M.~Wendt~\cite{Wendt} which showed that the centrality of $\mathrm K_2$ does {\it not} hold in general for root systems of rank $2$, i\,e. root systems of type $\rA_2$, $\rB_2$ and $\rG_2$ (similarly, the Suslin's normality theorem fails for the rank $1$ group $\mathrm{SL}_2(R)$). Soon the papers~\cite{Lav,Sin,LavSin} of the first-named and the second-named authors appeared, in which it was shown that the centrality of $\mathrm K_2$ does hold for the Chevalley groups of type $\rC_\ell$, $\rD_\ell$, $\rE_\ell$ provided $\ell\geq 3$. In~\cite{Lav} the first-named author has developed a symplectic analogue of the technique of~\cite{vdK} while in~\cite{Sin, LavSin} an amalgamation theorem for relative Steinberg groups was developed which allowed one to reduce the problem of the centrality of $\mathrm K_2$ in the simply-laced case to the already-known linear case. Nevertheless, neither of these two approaches seemed to work for the root systems $\rB_\ell$ and $\rF_4$. Finally, in~\cite{Vor1} the third-named author has developed a novel pro-group approach, which allowed him not only to generalize~\cite{vdK,Tul} to even larger class of noncommutative rings, but also to prove the centrality of $\mathrm K_2$ for odd unitary groups and, thus, cover all classical groups, including the case $\rB_\ell$, see~\cite{Vor2}. In the present paper these methods are employed to verify the centrality of $\mathrm K_2$ for an arbitrary commutative ring $R$ in the case $\Phi = \rF_4$, the only case which remained open until now, see~\cref{SteinbergCrossedModule}. Our method of proof also applies to root systems of type $\rA_{\geq 3}$, $\rD_{\geq 4}$, $\rE_{6,7,8}$, and thus gives a proof of the centrality of $\mathrm{K}_2$, which is independent of the earlier results~\cite{vdK,Sin,LavSin}. In course of the proof we also reprove Taddei's normality theorem~\cite{Ta86} in the specified cases. Thus, the main consequence of \cite{Vor1,Vor2} and our~\cref{SteinbergCrossedModule} can be formulated as follows.
\begin{theorem*}
...
As a corollary, we obtain that Stein's $\mathrm K_2$-group $\mathrm{K}_2(\Phi, R)$ defined as the the kernel of the homomorphism~\eqref{eq:st} coincides with the unstable $\mathrm{K}_2$-group defined via Quillen's $+$-construction applied to the perfect subgroup $\mathrm{E}(\Phi, R)$: \[\mathrm{K}_2(\Phi, R) \cong \mathrm{K}_2^Q(\Phi, R) := \pi_2\left(\mathrm{BG}_{sc}(\Phi,\,R)^+_{\mathrm{E}(\Phi, R)}\right).\] The latter group coincides with the second homology group $\mathrm H_2(\mathrm E(\Phi,\,R),\,\mathbb Z)$; see e.\,g.~\cite[\S~IV.1]{Weibel}.
...
\[ \xymatrix{ 1\ar@{->}[r]&{\mathrm H_2(\mathrm{St}(\Phi,\,R),\mathbb Z)}\ar@{->}[r]&\mathrm K_2^Q(\Phi,\,R)\ar@{->}[r]&\mathrm K_2^S(\Phi,\,R)\ar@{->}[r]&1,} \text{where} \]
\begin{align*}
