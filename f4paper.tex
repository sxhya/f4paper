...
There is a group homomorphism $\St(\Phi, R^{(s)}) \to \St(\Phi, sR)$ given by $x_\alpha^{(s)}(a)\mapsto x_\alpha(sa)$. By~\cref{rem:prorings-comment} this homomorphism is isomorphism in the case when $R$ is an integral domain. Thus, similarly to pro-rngs, Steinberg pro-groups often do not have global elements.
\end{rem}
...
\begin{df}
 ...
 $\GG^{(s)}(\Phi, R) = \GG(\Phi, R^{(s)} \rtimes R, R^{(s)}) = \Ker\left(\GG_{sc}(\Phi, R^{(s)} \rtimes R) \to \GG_{sc}(\Phi, R)\right)$ with the structure morphisms induced by the structure morphisms of \(R^{(\infty)}\). Define the {\it simply-connected Chevalley pro-group} $\GG^{(\infty)}(\Phi, R)$ as the formal projective limit of this system.
\end{df}

\begin{rem}
 The above definition may be formulated in terms of Hopf algebras. Let \(H\) be the Hopf \(\ZZ\)-algebra corresponding to \(\GG_{sc}(\Phi, -)\), \(\varepsilon \colon H \to \ZZ\) be the counit, \(\Delta \colon H \to H \otimes H\) be the comultiplication, and \(S \colon H \to H\) be the antipode. %Then there is a canonical isomorphism \(\GG^{(s)}(\Phi, R) \cong \Rng(\Ker(\varepsilon), R^{(s)})\). The inverse on this group is the pre-composition with \(S \colon \Ker(\varepsilon) \to \Ker(\varepsilon)\), the neutral element is the zero map \(0 \colon \Ker(\varepsilon) \to R^{(s)}\), and the group product of \(g, h \in \Rng(\Ker(\varepsilon), R^{(s)})\) is \(g \odot h + g + h\), where \(g \odot h\) is the composition
 %\[\Ker(\varepsilon) \xrightarrow{\widetilde \Delta} \Ker(\varepsilon) \otimes \Ker(\varepsilon) \xrightarrow{g \otimes h} R^{(s)} \otimes R^{(s)} \to R^{(s)}\]
 %and \(\widetilde \Delta \colon \Ker(\varepsilon) \to \Ker(\varepsilon) \otimes \Ker(\varepsilon), x \mapsto \Delta(x) - x \otimes 1 - 1 \otimes x\).
 Note that \(\Ker(\varepsilon)\) is a Hopf object in \(\Rng\) with respect to the monoidal operation \(R \otimeshat S = R \oplus S \oplus R \otimes S\) (i.e. the coproduct of rngs). Then there is a canonical isomorphism \(\GG^{(s)}(\Phi, R) \cong \Rng(\Ker(\varepsilon), R^{(s)})\). The inverse on this group is the pre-composition with \(S \colon \Ker(\varepsilon) \to \Ker(\varepsilon)\), the neutral element is the zero map \(0 \colon \Ker(\varepsilon) \to R^{(s)}\), and the group product of \(g, h \in \Rng(\Ker(\varepsilon), R^{(s)})\) is the convolution in \(\Rng\), i.e. the composition
...
\begin{rem}
 To formulate the next lemma, we need to introduce a way of reinterpreting the usual Steinberg relations~\eqref{R1}--\eqref{R4} as relations between the morphisms $x_\alpha$ in the category $\Pro(\Set)$. We call these relations the {\it pro-analogues} of~\eqref{R1}--\eqref{R4}. To obtain these relations one has to add the upper index $(\infty)$ to the variables $a$, $b$ occuring in~\eqref{R1}--\eqref{R4} and then read the resulting expressions according to~\cref{conv:notation}. 
 Thus, for example, the pro-analogue of~\eqref{R1} is the identity $x_\alpha(a^{(\infty)}) x_\alpha(b^{(\infty)}) = x_\alpha(a^{(\infty)} + b^{(\infty)})$, or in the usual notation $m (x_\alpha \times x_\alpha) = x_\alpha (+_{R^{(\infty)}})$. It is clear that this relation is automatically satisfied by $x_\alpha$ for every $\alpha\in \Phi$.
\end{rem}
...
\begin{rem} \label{rem:xpma-presentation}
 One can obtain the analogue of~\cref{SteinbergPresentation} for the pro-group $\St^{(\infty)}(\Phi\setminus\{\pm\alpha\}, R)$ by repeating the same proof verbatim. In particular, $x_\beta$, $\beta\neq \pm\alpha$ ``generate'' $\St^{(\infty)}(\Phi\setminus\{\pm\alpha\}, R)$ in the categorical sense.
\end{rem}
There is a morphism of pro-groups
$F_\alpha \colon \St^{(\infty)}(\Phi \setminus\{\pm\alpha\}, R) \to \St^{(\infty)}(\Phi, R).$
given by the pre-morphism $F_\alpha^{*} = \mathrm{id}_S$ and the homomorphisms \[F_\alpha^{(s)} \colon \St^{(s)}(\Phi \setminus\{\pm\alpha\}, R) \to \St^{(s)}(\Phi, R)\] induced by the obvious embedding of generators.
...
\end{itemize}
Here $\mathrm e_i$ denote the standard basis vectors of $\mathbb{R}^3$ and we assume that the root systems $\rA_3$ and $\rC_3$ are realized in as in~\cite[Ch.~VI,~\S\S~4.6--4.7]{Bou81}. 
...
By~\cref{SteinbergPresentation} the morphisms $\widetilde{x}_{\beta}$ give rise to a morphism $G_\alpha \colon \St^{(\infty)}(\Phi, R) \to \St^{(\infty)}(\Phi\setminus\{\pm\alpha\}, R)$. By definition, $G_\alpha x_\beta = \widetilde{x}_\beta$ for all $\beta$. On the other hand, it is clear that $F_\alpha \widetilde{x}_\beta = x_\beta$ for $\beta\neq \pm\alpha$. To see that $G_\alpha$ is inverse to $F_\alpha$ it remains to use the first part of~\cref{SteinbergPresentation} and~\cref{rem:xpma-presentation}.
\end{proof}


 \begin{theorem}\label{DoubleRootElimination}
  Let \(R\) be a ring, \(S \subseteq R\) be a multiplicative subset, \(\Phi\) be a root system of rank \(\geq 3\) different from \(\rB_l\) and \(\rC_l\), \(\Phi_0 \subseteq \Phi\) be a subsystem of rank \(2\) (i.e. \(\Phi_0 = \Phi \cap (\mathbb R\alpha + \mathbb R\beta)\) for linearly independent \(\alpha, \beta \in \Phi\)). Then 
  \[\St^{(\infty)}(\Phi \setminus \Phi_0, R) \to \St^{(\infty)}(\Phi, R)\] is an epimorphism of pro-groups.
 \end{theorem}
 \begin{proof}
...
The pro-ring $R^{(\infty)}$ can be turned into an \(S^{-1} R\)-algebra in the following manner. For every class of fractions \([\frac rs] \in S^{-1} R\) there is a well-defined pro-group endomorphism \(m_{[\frac rs]} \colon R^{(\infty)} \to R^{(\infty)}\) given by \(m_{[\frac rs]}^*(s') = ss'\) and \(m_{[\frac rs]}^{(s')}(a^{(ss')}) = (ra)^{(s')}\). It is easy to see that $m_{[\frac{r}{s}]}$ does not depend on the choice of the representative \(\tfrac r s\). Moreover, it is easy to see that \(m\) is actually a homomorphism of rings \(S^{-1} R \to \Pro(\Group)(R^{(\infty)})\) and that \[m_{[\frac rs]}(a^{(\infty)} b^{(\infty)}) = m_{[\frac rs]}(a^{(\infty)}) b^{(\infty)} = a^{(\infty)} m_{[\frac rs]}(b^{(\infty)}).\] For shortness we write \(\frac rs a^{(\infty)}\) instead of \(m_{[\frac rs]}(a^{(\infty)})\). 

Similarly, \(\GG_{sc}(\Phi, S^{-1} R)\) acts on the Chevalley pro-group \(\GG^{(\infty)}(\Phi, R)\) by automorphisms. Indeed, denote by \(H\) the Hopf \(\ZZ\)-algebra of \(\GG_{sc}(\Phi, -)\) with the structure of a Hopf algebra given by the triple $(\Delta, S, \varepsilon)$. Recall that \(H\) is a finitely presented commutative \(\ZZ\)-algebra. The ``co-conjugation'' map
\[H \xrightarrow{\Delta^{(2)}} H \otimes H \otimes H \xrightarrow{\mathrm{id} \otimes \mathrm{id} \otimes S} H \otimes H \otimes H\]
induces a map \(\mathrm{Conj} \colon \Ker(\varepsilon) \to H \otimes \Ker(\varepsilon) \otimes H\). Take \(g \in \mathbf{Ring}(H, S^{-1}R) \cong \GG_{sc}(\Phi, S^{-1} R)\) and a sufficiently large \(s \in S\) depending on \(g\) (see below). For any \(h \in \Rng(\Ker(\varepsilon), R^{(ss')}) \cong \GG^{(ss')}(\Phi, R)\) denote by \(\up gh\) the composition
\[\Ker(\varepsilon) \xrightarrow{\mathrm{Conj}} H \otimes \Ker(\varepsilon) \otimes H \xrightarrow{g \otimes h \otimes g} S^{-1} R \otimes R^{(ss')} \otimes S^{-1} R \dashrightarrow R^{(s')}.\]
Here the last map is only partially defined. But if \(s\) is large enough, then the composition is defined on a finite generating set of \(\Ker(\varepsilon)\), maps finitely many relations between them to \(0\), and gives a group homomorphism \(\GG^{(ss')}(\Phi, R) \to \GG^{(s')}(\Phi, R)\). It may be directly checked that this data gives the required action.

\begin{df}\label{root-action}
 Let \(R\) be a ring, \(S \subseteq R\) be a multiplicative subset, and \(\Phi\) be a root system of rank \(\geq 3\) different from \(\rB_l\) and \(\rC_l\). For every root \(\beta \in \Phi\) and every \(g\) in the standard torus \(\Torus(\Phi, S^{-1} R)\) of \(\GG_{sc}(\Phi, S^{-1} R)\) let
