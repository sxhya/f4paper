...
Similarly, \(\GG_{sc}(\Phi, S^{-1} R)\) acts on the Chevalley pro-group \(\GG^{(\infty)}(\Phi, R)\) by automorphisms. Indeed, denote by \(H\) the Hopf \(\ZZ\)-algebra of \(\GG_{sc}(\Phi, -)\) with the structure of a Hopf algebra given by the triple $(\Delta, S, \varepsilon)$. Recall that \(H\) is a finitely presented commutative \(\ZZ\)-algebra. There is a map
\[\mathrm{Conj} \colon \Ker(\varepsilon) \to H \otimes \Ker(\varepsilon), h \mapsto \sum h_{(1)} S(h_{(3)}) \otimes h_{(2)}\]
in Sweedler notation. Take \(g \in \mathbf{Ring}(H, S^{-1}R) \cong \GG_{sc}(\Phi, S^{-1} R)\) and a sufficiently large \(s \in S\) depending on \(g\) (see below). For any \(h \in \Rng(\Ker(\varepsilon), R^{(ss')}) \cong \GG^{(ss')}(\Phi, R)\) denote by \(\up gh\) the composition
\[\Ker(\varepsilon) \xrightarrow{\mathrm{Conj}} H \otimes \Ker(\varepsilon) \xrightarrow{g \otimes h} S^{-1} R \otimes R^{(ss')} \dashrightarrow R^{(s')}.\]
