...
\begin{df}
 ...
 $\GG(\Phi, R^{(s)} \rtimes R, R^{(s)}) = \Ker\left(\GG_{sc}(\Phi, R^{(s)} \rtimes R) \to \GG_{sc}(\Phi, R)\right)$ with the structure morphisms induced by the structure morphisms of \(R^{(\infty)}\). Define the {\it simply-connected Chevalley pro-group} $\GG^{(\infty)}(\Phi, R)$ as the formal projective limit of this system. The definition may be formulated in terms of Hopf algebras. Let \(H\) be the Hopf \(R\)-algebra corresponding to \(\GG_{sc}(\Phi, R)\) and \(\varepsilon \colon H \to R\) be the augmentation. Then \(\GG^{(s)}(\Phi, R) \cong \Rng_R(\Ker(\varepsilon), R^{(s)})\), where \(\Rng_R\) is the category of commutative non-unital \(R\)-algebras. This isomorphism commutes with the structure morphisms of the pro-objects.
\end{df}
...
To help distinguish between the root subgroup morphisms of $\St^{(\infty)}(\Phi, R)$ and $\St^{(\infty)}(\Phi\setminus\{\pm\alpha\}, R)$.
For the sake of brevity let us rename the root subgroup morphisms $x_\beta$, $\beta\neq\pm\alpha$ of $\St(\Phi\setminus\{\pm\alpha\}, R^{(\infty)})$ to $\widetilde{x}_{\beta}$. For the morphisms $x_\beta$ of $\St^{(\infty)}(\Phi, R)$ we continue to use the usual notation. 
