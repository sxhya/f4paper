...
Let $\Sigma \subseteq \Phi$ be a special root subset. It is well-known that the restriction of the canonical map $\St(\Phi, R)\to\GG_{\mathrm{st}}(\Phi, R)$ to the subgroup \[\mathrm{U}(\Sigma, R) = \langle x_{\alpha}(a) \mid a\in R,\ \alpha\in \Sigma \rangle \leq \St(\Phi, R)\] is injective. Morever, for any chosen linear order on \(\Sigma\) the map $R^{\Sigma} \to \mathrm{U}(\Sigma, R)$ given by $(r_\alpha)_\alpha \mapsto \prod_\alpha x_\alpha(r_\alpha)$ is a bijection.
%There is the obvious pro-group analogue \(\mathrm U^{(\infty)}(\Sigma, R)\) with a morphism \(\mathrm U^{(\infty)}(\Sigma, R) \to \St^{(\infty)}(\Phi, R)\) of pro-groups 
%(it seems to be a monomorhism, though we do not need this property)
%and an isomorphism \(\mathrm U^{(\infty)}(\Sigma, R) \cong (R^{(\infty)})^\Sigma\) of pro-sets.
\end{rem}
...
\begin{rem}
 ...
 Now if $R$ is an arbitary ring and $s \in R$ then there is a canonical isomorphism \(\GG(\Phi, R^{(s)}) \cong \Rng(\Ker(\varepsilon), R^{(s)})\). The structure of a group on $\GG(\Phi, R^{(s)})$ is then induced from the Hopf monoid structure on $\Ker(\varepsilon)$ by duality.
...
\end{prop}
\begin{proof} 
 Let us check that the root subgroup morphisms \[\{\up{x_\alpha(u)} x_\beta \colon R^{(\infty)} \to \St^{(\infty)}(\Phi\setminus\{\pm\alpha\}, R) \mid \beta \neq \pm \alpha \}\] defined above satisfy the defining relations of $\St^{(\infty)}(\Phi\setminus\{\pm \alpha\}, R)$. Clearly, all of them are morphisms of pro-groups. Take roots \(\beta, \gamma \in \Phi \setminus \{\pm \alpha\}\) such that there is Steinberg relation for \([x_\beta(b^{(\infty)}), x_\gamma(c^{(\infty)})]\) in \(\St^{(\infty)}(\Phi \setminus \{\pm \alpha\}, R)\). Consider the ``universal'' identity
 ...
 in \(\St(\Phi, \mathbb Z[b, c, t])\), where \(N_{\beta, \gamma}^{1, 1} = N_{\beta, \gamma}\) and \(N_{\beta, \gamma}^{1, 2} = -N_{\gamma, \beta}^{2, 1}\). This identity follows from Steinberg relations in \(\St(\Phi \setminus \{\pm \alpha\}, \mathbb Z[b, c, t])\) and the Steinberg relations for \([x_\alpha(t), x_\delta(d)]\), where \(\delta \in \Phi \setminus \{\pm \alpha\}\) and \(d \in b \mathbb Z[b, c, t] + c \mathbb Z[b, c, t]\). Applying the pro-analogs of these relations, we get
 \[[\up{x_\alpha(u)}{x_\beta(b^{(\infty)})}, \up{x_\alpha(u)}{x_\gamma(c^{(\infty)})}] = \prod_{\substack{i\beta + j\gamma \in \Phi\\ i, j > 0}} \up{x_\alpha(u)}{x_{i\beta + j\gamma}\bigl(N_{\beta, \gamma}^{i, j} \bigl(b^{(\infty)}\bigr)^i \bigl(c^{(\infty)}\bigr)^j\bigr)}.\]
 %Then there is a special root subset \(\Sigma \subseteq \Phi\) containing all the roots \(\alpha, \beta, \gamma\), and the required relations are some identities in the pro-group \(\mathrm U^{(\infty)}(\Sigma, R) \cong (R^{(\infty)})^\Sigma\) (see \cref{rem:uni-rad}). But then these identities follows from the ``universal'' identity
 %\[[\up{x_\alpha(t)}{x_\beta(b)}, \up{x_\alpha(t)}{x_\gamma(c)}] = \up{x_\alpha(t)}{[x_\beta(b), x_\gamma(c)]}\]
 %in \(\St(\Phi, \mathbb Z[b, c, t])\), the definitions, and the Steinberg relations.
