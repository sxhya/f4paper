...
%  Notice that the root subset $\Sigma = \Phi \cap (\ZZ_{\geq 0}\beta + \ZZ_{\geq 0}\gamma + \ZZ_{>0}\delta)$ is special and does not contain $\pm\alpha$, $-\beta$ or $-\gamma$. Thus, the relations between the remaining root subgroup morphisms of $\St^{(\infty)}(\Phi\setminus\{\pm\alpha\}, R)$ suffice to rewrite the left-hand side of~\eqref{R2-bg}--\eqref{R4-bg2} as follows:
 \begin{align}
  &\bigl[x_{\beta, \gamma}(b^{(\infty)}, c^{(\infty)}), x_\delta(d^{(\infty)})\bigr] = \label{eq:24_1} \\
  %
 ...
 \end{align}
 The integers \(A_{i, j, k}\) depend only on the roots \(\beta\), \(\gamma\), \(\delta\) and the chosen order of factors.
 They can be determined from the following identity in $\St(\Phi, \mathbb{Z})$ (we assume that the order of factors is the same):
 \begin{align*}
  [x_\alpha(1), x_\delta(1)] = \prod_{\substack{i\beta + j\gamma + k\delta \in \Phi\\ i, j \geq 0; k > 0}}
  x_{i\beta + j\gamma + k\delta}(A_{i, j, k}).
 \end{align*}
 The constants $A_{i,j,k}$ coincide with the constants in the right-hand sides of~\eqref{R2}--\eqref{R4}, in particular, $A_{i,j,k}=0$ for $i\neq j$. Thus, in the case under consideration, \eqref{R2-bg}--\eqref{R4-bg2} follow from~\eqref{eq:24_1}.
 ...
 in the Steinberg group \(\St(\Phi, \mathbb Z)\). In this Steinberg group the commutant is trivial, hence by \cref{rem:uni-rad} we have \(B_{i, j, k} = 0\) for all \(i, j, k\). Then
...
\begin{proof}
 ...
 From \cref{rem:uni-rad} we have \(P_{i, j}(a) = 0\) with the only exception \(P_{0, 0}(a) = a^{(\beta, \alpha)}\).
\end{proof}
