...
First, a formal variable was thrown in and the original question was reduced to the question of the triviality of $[g, x_{ij}(t)]$ in $\St_n(R[t])$. Then via a trick the problem was reduced to proving the {\it dilation principle} on principal localizations (see e.\,g.~\cite[Lemma~15]{Sin}), whose proof, in turn, was based on the isomorphism of {\it relative Steinberg groups} $\St_n(R_a[t], tR_a[t]) \cong \St_n(R \ltimes R_a[t], tR_a[t])$; cf.~\cite[Remark~3.12]{Sin}. Essentially this isomorphism means that it possible to define a conjugation action of $\St_n(R_a)$ on the group $\St_n(R\ltimes R_a[t], tR_a[t])$.
This last step, however, required finding nontrivial ad hoc presentations of relative Steinberg groups.
For example~\cite{Tul, Lavlgp} relied on a variant of van der Kallen's ``another presentation''~\cite{vdK} for linear and symplectic groups, while~\cite{Sin, LavSin} used the amalgamation theorem for relative Steinberg groups; see~\cite[Theorem~9]{Sin}. However, it was not easy to transfer to all Chevalley groups either of these approaches.
...
It turns out that it is possible to naturally define the ``conjugation'' action of the Steinberg group $\St(\Phi, R_a)$ on such a sequence. Naively, for each $g \in \St(\Phi, R_a)$ the automorphism of conjugation by $g$ corresponds to a collection of group homomorphisms \[\{\St(\Phi, a^{m_2}R) \to \St(\Phi, a^{m_1}R)\}_{m_1 \in \mathbb N },\] where $m_2$ depends on $g$ and $m_1$.
...
For each $x_{\alpha}(a/s)$, $a\in R$, $s\in S = R\setminus M$ the automorphism of conjugation by $x_\alpha(a/s)$ would correspond to a certain collection of group homomorphisms \[\{conj_{x_\alpha(a/s)}(t) \colon  \St(\Phi, c^*(t)R) \to \St(\Phi, tR)\}_{t \in S},\] where $c^*\colon S \to S$ is the function $t \mapsto ts$, called the {\it index function}.

In fact, a morphism between pro-groups should be defined not just as a collection of homomorphisms, but rather as an {\it equivalence class} of such collections. By definition, two pro-group morphisms $m_1$ and $m_2$ with index functions $m_1^*$,$m_2^* \colon S \to S$ are equivalent, if for every $t$ one can choose $s$ divisible both by $m_1^*(t)$ and $m_2^*(t)$ so that the homomorphisms \[m_i(t) \colon \St(\Phi, m_i^*(t)R) \to \St(\Phi, tR),\ i=1,2\] coincide after precomposition with the structure morphisms \[\St(\Phi, sR) \to \St(\Phi, m^*_i(t)R),\ i=1,2.\]
...
Thus, one can define the pro-group automorphism $conj_{\lambda_M(g)}$ of conjugation by $\lambda_M(g)$ as a suitable composition of $conj_{x_{\alpha_i}(a_i/s_i)}$.
Moreover, for $g\in \St(\Phi, R)$ this pro-group automorphism is compatible with the usual conjugation action of $g$,
i.\,e. for some $s_0 \in S$ the group homomorphism $conj_{\lambda_M(g)}(1) \colon \St(\Phi, s_0R) \to \St(\Phi, R)$ coincides with the obvious homomorphism $h \mapsto ghg^{-1}$. Since our conjugation action factorizes through $\St(\Phi, S^{-1}R)$ we obtain that the conjugation by $g \in \mathrm K_2(\Phi, R)$ acts trivially on $x_\alpha(s_0R)$ and we are done.
...
We denote by \[\pi_R \colon R^{(\infty)} \to R,\ \pi_{\St} \colon \St^{(\infty)}(\Phi, R) \to \St(\Phi, R)\text{ and } \pi_{\GG} \colon \GG^{(\infty)}(\Phi, R) \to \GG(\Phi, R).\] the canonical pro-group morphisms. Notice that the action of $h\in \GG_{\mathrm{sc}}(\Phi, R)$ the pro-group $\GG^{(\infty)}(\Phi, R)$ is compatible with the conjugation action of $\GG_{\mathrm{sc}}(\Phi, R)$ on itself, i.\,e. $\up{h}\pi_{\GG}(g^{(\infty)}) = \pi_{\GG}(\up{h} {g^{(\infty)}})$.
...
\begin{proof}
 ...
 this action is preserved by $\mathrm{st}$, we obtain the following equality of morphisms $R^{(\infty)} \to \GG_{\mathrm{sc}}(\Phi, R)$:
 \[
...
\end{theorem}
\begin{proof}  
 Notice that under our assumptions the group $\E(\Phi, R)$ is perfect, therefore by~\cite[Lemma~1.1]{St71} for every $g\in \GG_{\mathrm{sc}}(\Phi, R)$ there may exist at most one endomorphism $\up g (-)$ of $\St(\Phi, R)$ satisfying 
