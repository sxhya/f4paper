...
%Any algebraic formula (say, a commutator or a polynomial with integer coefficients) defines a morphism in \(\Pro(\Set)\) from a product of copies of an algebraic object to this algebraic object. If \(a^{(\infty)}\) is a variable in such a formula, then the notation \(a^{(\infty)} \in X^{(\infty)}\) means that \(X^{(\infty)}\) is the domain of \(a^{(\infty)}\).
...
\begin{rem}
 ...
 The reason why homotopes rather than principal ideals were used in the definition of $R^{(\infty)}$ is that
 there is always a ``division by $s$'' homomorphism of $R$-algebras $R^{(ss')} \to R^{(s')}$ given by $r^{(ss')} \mapsto r^{(s')},$
 while a similar map $ss'R \to s'R$ may fail to exist if $R$ does not happen to be a domain.
 
 Notice also that the projective limit of $R^{(s)}$ is often trivial in $\Rng$.
 ...
 This also shows that the global elements of $R^{(\infty)}$ (in $\Fun(\mathcal{S}^{\op}, \Set)$ or $\Pro(\Set)$) are usually of little interest.
\end{rem}

It will be convenient for us to use the notation \(a^{(s)} \otimes b^{(s)}\) for the elements of \(R^{(s)} \times R^{(s)}\) and the notation \(a^{(\infty)} \otimes b^{(\infty)}\) for the elements of \(R^{(\infty)} \times R^{(\infty)}\). We denote by $a^{(\infty)} b^{(\infty)}$ the image of \(a^{(\infty)} \otimes b^{(\infty)}\) under the rng multiplication morphism  \(R^{(\infty)} \times R^{(\infty)} \to R^{(\infty)}\).
