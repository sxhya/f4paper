...
As a corollary, we obtain that Stein's $\mathrm K_2$-group $\mathrm{K}^{S}_2(\Phi, R)$ defined as the the kernel of the homomorphism~\eqref{eq:st} coincides with the unstable $\mathrm{K}_2$-group defined via Quillen's $+$-construction applied to the perfect subgroup $\mathrm{E}(\Phi, R)$: \[\mathrm{K}_2(\Phi, R) \cong \mathrm{K}_2^Q(\Phi, R) := \pi_2(\mathrm{BG}_{sc}(\Phi,\,R)^+_{\mathrm{E}(\Phi, R)}).\] The latter group coincides with the second homology group $\mathrm H_2(\mathrm E(\Phi,\,R),\,\mathbb Z)$; see e.\,g.~\cite[\S~IV.1]{Weibel}.
Thus, combining the above theorem with the known results~\cite{St71,vdKStein} we obtain the following.
\begin{corollary*}
For an arbitrary commutative ring $R$ and a root system $\Phi=\mathrm \rA_\ell,\rB_\ell,\rC_\ell$ for $\ell\geq4$, $\Phi=\mathrm \rD_\ell$ for $\ell\geq5$, or $\Phi=\mathrm \rE_6,\,\mathrm \rE_7,\,\mathrm \rE_8$ one has $\mathrm K_2^S(\Phi,\,R)=\mathrm K_2^Q(\Phi,\,R)$. Moreover, for $\Phi=\rA_3,\rB_3,\rC_3,\rD_4,\rF_4$ the following exact sequences hold:
\[ \xymatrix{ 1\ar@{->}[r]&{\mathrm H_2(\mathrm{St}(\Phi,\,R),\mathbb Z)}\ar@{->}[r]&\mathrm K_2^Q(\Phi,\,R)\ar@{->}[r]&\mathrm K_2^S(\Phi,\,R)\ar@{->}[r]&1,} \]
where
\begin{align*}
...
\end{align*}
In particular, for $\Phi=\mathrm A_3,\mathrm C_3,\mathrm D_4,\mathrm F_4$ the equality $\mathrm K_2^S(\Phi,\,R)=\mathrm K_2^Q(\Phi,\,R)$ holds iff $R$ does not have residue fields isomorphic to $\mathbb F_2$ and $\mathrm K_2^S(\mathrm B_3,\,R)=\mathrm K_2^Q(\mathrm B_3,\,R)$ iff $R$ does not have residue fields isomorphic to $\mathbb F_2$ и $\mathbb F_3$.
\end{corollary*}
