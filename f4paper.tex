...
Since the proof of the main theorem is rather long and technical and the setting of the papers~\cite{Vor1,Vor2} is already rather general, let us briefly sketch the key ideas of our proof using the simplest case of the functor $\mathrm{K}_{2,n}$ as an example.
Already in this form our proof can be seen to be influenced by the Quillen--Suslin local-global principle.
...
The naive expectation why~(\ref{intro}) must hold is as follows: if we take any presentation $g=\prod_hx_{k_h l_h}(r_h)$ for some $r_h\in R$, then after localization at $M$ the equality $[\prod_hx_{k_h l_h}(r_h),\,x_{ij}(s_0r)]=1$ will be a consequence of a finite number of Steinberg relations. These relations involve denominators from $R\setminus M$, but it seems possible to choose $s_0$ so large that all the denominators in all the Steinberg relations involved disappear after reducing the fractions. This would mean that the relation $[g,\,x_{ij}(s_0r)]=1$ already holds in the Steinberg group $\St_n(R)$.

Unfortunately, it is not that easy to formalize the last argument. Clasically, this formalization looked as follows (cf.~\cite{Tul,Sin,LavSin,Lavlgp}):
first, the localization at a maximal ideal was reduced to the localization at some principal ideal $sR$ (cf, e.\,g. the proof of ~\cite[Theorem~2]{Sin}), ...
