...
Recall that \(S^{-1} R\) is isomorphic as an $R$-module to the filtered colimit of \(R\)-modules \(\frac Rs\) over the category $\mathcal{S}$ (cf.~\cref{sec:rng-homotopes}), each of these modules being isomorphic to $R$. For $s, t \in S$ the homomorphism \(\frac R{s} \to \frac R{st}\) is given by $\tfrac{r}{s} \mapsto \tfrac{rt}{st}$. The elements of modules $\frac Rs$ can be ``multiplied'' with each other via the natural $R$-module homomorphisms $\frac R s \otimes_R \frac R t \to \frac R {st}$ compatible with the canonical homomorphisms $\frac R s \to S^{-1}R$.
...
Since tensor products commute with colimits, there exists \(s_0 \in S\) such that\((g \otimes \mathrm{id})(a_i)\) are the images of certain elements $\up g {a_i}$ of \(\frac R{s_0} \otimes \Ker(\varepsilon)$ under the canonical homomorphism \[ \tfrac R{s_0} \otimes \Ker(\varepsilon) \to S^{-1}R \otimes \Ker(\varepsilon).\] It is easy to see that there exists \(s_1 \in S s_0^N\) such that in $\frac R{s_1} \otimes \Ker(\varepsilon)$ we have the identities \(f_j(\up g{a_1}, \ldots, \up g{a_n}) = 0\) for all \(j\)'s.

 There is also a canonical isomorphism \(\frac Rs \otimes_R R^{(ss')} \cong R^{(s')}, \frac rs \otimes {r'}^{(ss')} \mapsto (rr')^{(s')}\) for every \(s, s' \in S\), so the \(R\)-module \(\frac Rs \otimes_R R^{(ss')}\) has a natural multiplication.
