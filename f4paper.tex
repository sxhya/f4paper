...
\begin{corollary*}
For an arbitrary commutative ring $R$ and a root system $\Phi$ of rank $\geq 3$ there is an exact sequence \[ \xymatrix{ 1\ar@{->}[r]&{\mathrm H_2(\mathrm{St}(\Phi,\,R),\mathbb Z)}\ar@{->}[r]&\mathrm K_2^Q(\Phi,\,R)\ar@{->}[r]&\mathrm K_2(\Phi,\,R)\ar@{->}[r]&1,}\] in which the group $\mathrm H_2(\mathrm{St}(\Phi, R), \ZZ)$ is trivial in the cases 
