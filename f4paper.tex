...
For example~\cite{Tul, Lavlgp} relied on a variant of van der Kallen's ``another presentation''~\cite{vdK} for linear and symplectic groups, while~\cite{Sin, LavSin} used the amalgamation theorem for relative Steinberg groups; see~\cite[Theorem~9]{Sin}. However, it was not easy to transfer to all Chevalley groups either of these two approaches.
...
It turns out that it is possible to naturally define the ``conjugation'' action of the Steinberg group $\St(\Phi, R_a)$ on such a sequence. Naively, for $g \in \St(\Phi, R_a)$ the automorphism of conjugation by $g$ corresponds to a collection of group homomorphisms \[\{\St(\Phi, a^{m_2}R) \to \St(\Phi, a^{m_1}R)\}_{m_1 \in \mathbb N },\] where $m_2$ is some function depending on $g$ and $m_1$.
