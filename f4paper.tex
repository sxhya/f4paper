...
\address{St. Petersburg State University, 14th Line V.O., 29, Saint Petersburg 199178 Russia}

\date {\today}

\begin{document}
\maketitle
\begin{abstract} We prove the centrality of $\mathrm K_2(\Phi, R)$ for an arbitrary commutative ring $R$ and root system $\Phi$ of rank $\geq 3$. Our proof uses only elementary localization techniques reformulated in terms of pro-groups. \end{abstract}
...
Recall that the algebraic $\mathrm K_2$-functor was originally defined by J.~Milnor as the kernel of the homomorphism $\St_\infty(R) \to \mathrm{GL}_\infty(R)$, i.\,e. as the group of ``nontrivial'' relations between transvections in the infinite-dimensional general linear group. Also he showed that the linear Steinberg group $\St_n(R)$ over an arbitrary ring has trivial Schur multiplier for $n \geq 5$. This shows that $\St_n(R)$ is a good candidate for the role of the universal central extension of the elementary subgroup $\mathrm{E}_n(R) = \mathrm{Im}(\St_n(R) \to \mathrm{SL}_n(R))$. %The centrality of the homomorphism $\St_n(R) \to \mathrm{GL}_n(R)$ for finite $n$, however, was not studied in~\cite{Milnor}.
...
On the other hand, the analogue of Suslin's normality theorem for Chevalley groups of rank $\geq 2$, i.\,e. the normality of the subgroup $\E_{\mathrm{sc}}(\Phi, R):= \mathrm{Im}(\mathrm{st})$ for an arbitrary commutative ring $R$, was soon obtained by G.~Taddei, see~\cite{Ta86}. 
...
As a corollary, we obtain that Stein's $\mathrm K_2$-group $\mathrm{K}_2(\Phi, R)$ defined as the the kernel of the homomorphism~\eqref{eq:st} coincides with the unstable $\mathrm{K}_2$-group defined via Quillen's $+$-construction applied to the perfect subgroup $\mathrm{E}_{\mathrm{sc}}(\Phi, R)$: \[ \mathrm{K}_2^Q(\Phi, R) := \pi_2\mathrm{BG}_{\mathrm{sc}}(\Phi,\,R)^+_{\mathrm{E}_{\mathrm{sc}}(\Phi, R)}.\] The latter group also coincides with the second homology group $\mathrm H_2(\mathrm E_{\mathrm{sc}}(\Phi,\,R),\,\mathbb Z)$; see e.\,g.~\cite[\S~IV.1]{Weibel}.
Thus, combining the above theorem with the known results~\cite{St71,vdKStein} we obtain the following.
\begin{corollary*}
For an arbitrary commutative ring $R$ and a root system $\Phi=\mathrm \rA_\ell,\rB_\ell,\rC_\ell$ for $\ell\geq4$, $\Phi=\mathrm \rD_\ell$ for $\ell\geq5$, or $\Phi=\mathrm \rE_6,\,\mathrm \rE_7,\,\mathrm \rE_8$ one has $\mathrm K_2(\Phi,\,R)=\mathrm K_2^Q(\Phi,\,R)$. Moreover, for $\Phi=\rA_3,\rB_3,\rC_3,\rD_4,\rF_4$ the following exact sequences hold:
\[ \xymatrix{ 1\ar@{->}[r]&{\mathrm H_2(\mathrm{St}(\Phi,\,R),\mathbb Z)}\ar@{->}[r]&\mathrm K_2^Q(\Phi,\,R)\ar@{->}[r]&\mathrm K_2(\Phi,\,R)\ar@{->}[r]&1,}\]
where
\begin{align*}
\mathrm H_2\big(\mathrm{St}(\mathrm A_3,\,R),\mathbb Z\big)&=R/\langle2, (t^2-t)(s^2-s)\mid t,\,s\in R\rangle,\\
\mathrm H_2\big(\mathrm{St}(\mathrm B_3,\,R),\mathbb Z\big)&=R/\langle6, 3(t^2-t)(s^2-s), 2(t^3-t)\mid t,\,s\in R\rangle,\\
\mathrm H_2\big(\mathrm{St}(\mathrm C_3,\,R),\mathbb Z\big)&=R/\langle t^2-t\mid t\in R\rangle,\\
\mathrm H_2\big(\mathrm{St}(\mathrm D_4,\,R),\mathbb Z\big)&=\big(R/\langle t^2-t\mid t\in R\rangle\big)\times\big(R/\langle2,(t^2-t)(s^2-s)\mid t,s\in R\rangle\big),\\
\mathrm H_2\big(\mathrm{St}(\mathrm F_4,\,R),\mathbb Z\big)&=R/\langle t^2-t\mid t\in R\rangle.
\end{align*}
In particular, for $\Phi=\mathrm A_3,\mathrm C_3,\mathrm D_4,\mathrm F_4$ the equality $\mathrm K_2(\Phi,\,R)=\mathrm K_2^Q(\Phi,\,R)$ holds iff $R$ does not have residue fields isomorphic to $\mathbb F_2$ and $\mathrm K_2(\mathrm B_3,\,R)=\mathrm K_2^Q(\mathrm B_3,\,R)$ iff $R$ does not have residue fields isomorphic to $\mathbb F_2$ and $\mathbb F_3$.
\end{corollary*}

Since the proof of the main theorem is rather long and technical and the setting of the papers~\cite{Vor1,Vor2} is already rather general, let us briefly sketch the key ideas of our proof using the simplest case of the functor $\mathrm{K}_{2,n}$ as an example. For simplicity, we also assume that the ring $R$ is an integral domain.

The proof is based on a variant of Quillen--Suslin's local-global principle.
Consider an arbitrary element $g$ from $\mathrm K_{2,n}(R)$. We want to show that $g$ commutes with $x_{ij}(r)$ for every $r\in R$.
Consider the set \[I=\{a\in R\mid [g,\,x_{ij}(aR)]=1\},\] which is easily seen to be, in fact, an ideal. 
To prove the centrality of $\mathrm{K}_{2,n}(R)$ it suffices to show that $I=R$.
Assume the contrary and consider an arbitrary maximal ideal $M \trianglelefteq R$ containing $I$. 
By the centrality of $\mathrm K_{2,n}(R_M)$ one has the identity $[\lambda_{M}(g),\,x_{ij}(r)]=1$ for all $r\in R_{M}$ (we denote by $\lambda_M$ the localization homomorphism $R \to R_M$). 
If we were able to show for some $s_0\in R\setminus M$ that the identity \begin{equation} \label{intro} [g,\,x_{ij}(s_0R)]=1, \end{equation} holds in $\St(\Phi, R)$ or, in other words, that $s_0\in I\cap R\setminus M$, this would lead to a contradiction and hence would complete the proof of the centrality of $\mathrm K_{2,n}(R)$. %Naively one would hope to ``get rid of'' the denominators in these identities, i.\,e. to find $s_0 \in R\setminus M$ so large that the relation $[g,\,x_{ij}(s_0r)]=1$ becomes a consequence of Steinberg relations defining the group $\St_n(R)$.
Unfortunately, formally proving this is not easy. 

In the earlier papers the proof went as follows; cf.~\cite{Tul,Sin,LavSin,Lavlgp}.
First, a formal variable was thrown in and the original question was reduced to the question of the triviality of $[g, x_{ij}(t)]$ in $\St_n(R[t])$, which, in turn, was further reduced to proving the {\it Dilation principle}, a certain assertion about principal localizations (see e.\,g.~\cite[Lemma~15]{Sin}). The proof of the Dilation principle was based on the isomorphism of {\it relative Steinberg groups} $\St_n(R_a[t], tR_a[t]) \cong \St_n(R \ltimes R_a[t], tR_a[t])$; cf.~\cite[Remark~3.12]{Sin}. Essentially this isomorphism means that it possible to define a conjugation action of $\St_n(R_a)$ on the group $\St_n(R\ltimes R_a[t], tR_a[t])$.
...
A remarkable idea of the third-named author was to abandon relative Steinberg groups altogether and instead work with sequences of {\it unrelativized} Steinberg groups $\St(\Phi, a^nR)$, $n \in \mathbb N$ given by the usual Steinberg presentations.
...
For each $x_{\alpha}(a/s)$, $a\in R$, $s\in S = R\setminus M$ the automorphism of conjugation by $x_\alpha(a/s)$ corresponds to a collection of group homomorphisms \begin{equation} \label{eq:collection} \{conj_{x_\alpha(a/s)}(t) \colon  \St(\Phi, c^*(t)R) \to \St(\Phi, tR)\}_{t \in S},\end{equation} where $c^*\colon S \to S$ is the function $t \mapsto ts$, called the {\it index function}. The construction of the homomorphisms $conj_{x_\alpha(a/s)}(t)$ will be outlined below.
...
After the automorphisms of conjugation by $x_\alpha(a/s)$ are defined, the proof of~\eqref{intro} can be finished as follows.
We verify that these automorphisms satisfy the Steinberg relations defining $\St(\Phi, S^{-1}R)$ and, moreover, that the Steinberg  symbols $\{b_1, b_2\}$, $b_1, b_2 \in {S^{-1}R}^\times$ act trivially on the pro-group.
Thus, one can define the pro-group automorphism $conj_{\lambda_M(g)}$ of conjugation by $\lambda_M(g)$ as a suitable composition of $conj_{x_{\alpha_i}(a_i/1)}$.
Moreover, for $g\in \St(\Phi, R)$ this pro-group automorphism is compatible with the usual conjugation action of $g$,
i.\,e. for some $s_0 \in S$ the group homomorphism $conj_{\lambda_M(g)}(1) \colon \St(\Phi, s_0R) \to \St(\Phi, R)$ coincides with the obvious homomorphism $h \mapsto ghg^{-1}$. Since our conjugation action factorizes through $\St(\Phi, S^{-1}R)$ we obtain that the conjugation by $g \in \mathrm K_2(\Phi, R)$ acts trivially on $x_\alpha(s_0R)$ and we are done. This part of the proof is formalized in~\cref{sec:local-action,sec:proof-main}.

Now let us focus on the construction of the conjugation action of $\St(\Phi, S^{-1}R)$ on the Steinberg pro-group.
Let us also assume for simplicity that $\Phi$ is simply-laced.
In order to construct the automorphism of conjugation by $x_{\alpha}(a/s)$ we need to define the groups 
...
\[conj'_{x_\alpha(r/s)}(t)\colon \St(\Phi\setminus\{\pm\alpha\}, stR) \to \St(\Phi\setminus\{\pm\alpha\}, tR).\]

The obvious defect of the formula~\eqref{eq:conj-def} is that its correctness depends on the integrality of $R$.
However, it is easy to fix this using the technique of {\it homotopes of rings} introduced in~\cite{Vor1,Vor2}. Instead of working with groups over principal ideals $tR$ we work with groups over ``ring homotopes''. By definition, the homotope $R^{(t)}$ is the ring without unit, multiplication in which is given by the formula $x*y=txy$. The homotope $R^{(t)}$ is isomorphic to $tR$ as a ring without unit if $R$ happens to be a domain. This technique of homotopes allows one to formulate an analogue of~\eqref{eq:conj-def} for not necessarily integral rings and, thus, prove our main results without the need to first reduce to the case of domains. We also believe that this construction is interesting in its own right. Our main results, however, can be proved without ring homotopes; see~\cref{no-homotopes} for an outline of such a proof.

Although, the groups $\St(\Phi, tR)$ and $\St(\Phi\setminus\{\pm \alpha\}, tR)$ need not be isomorphic in general, the pro-groups composed of them turn out to be isomorphic in the category of pro-groups, see~\cref{SingleRootElimination}. Notice also that the corresponding assertion for unital rings, i.\,e. the isomorphism $\St(\Phi, R) \cong \St(\Phi\setminus\{\pm\alpha\}, R)$ is an easy corollary of a much stronger result called {\it Curtis--Tits} presentation; see e.\,g.~\cite[Corollary~1.3]{A13}. In fact, the main difference of the present paper from~\cite{Vor1, Vor2} is that our technique is based on Steinberg presentations omitting roots, while~\cite{Vor1, Vor2} rely on computations with relative root systems. 
...
The fact that this definition does not depend on the choice of $\beta$ can be checked using the Hall--Witt identity (notice that we can further decompose $x_{\alpha-\beta}(t^2b)$ into a commutator).
Similarly, we can verify that these commutators satisfy the relations missing from the presentation of $\St(\Phi\setminus\{\pm\alpha\}, t^3R)$. Keeping track of index functions and individual homomorphisms $F_\alpha(t)$ in such computations, however, is not very convenient, therefore, to simplify the proof of~\cref{SingleRootElimination}, we prefer to write down all commutator formula calculations inside the category of pro-groups. In addition, to reduce the number of cases that need to be considered, we also assume throughout the proof of~\cref{SingleRootElimination} that $\Phi$ is not of type $\rB_\ell$ or $\rC_\ell$. Now, combining the pro-group morphisms given by $F_\alpha(t)$ and $conj'_{x_\alpha(r/s)}(t)$ we obtain the sought pro-group morphism $conj_{x_\alpha(r/s)}$.

\subsection{Acknowledgements}
The first- and second-named authors were supported by ``Native towns'', a social investment program of PJSC ``Gazprom Neft''. The first-named author was also supported by Young Russian Mathematics award, and by Ministry of Science and Higher Education of the Russian Federation, agreement No. 075-15-2019-1619. The work of the second-named author was supported by RFBR grant No. 18-31-20044.
The third-named author was supported by the Russian Science Foundation grant 19-71-30002.
...
Let $\Sigma \subseteq \Phi$ be a special root subset. It is well-known that the restriction of the canonical map $\St(\Phi, R)\to\GG_{\mathrm{st}}(\Phi, R)$ to the subgroup \[\mathrm{U}(\Sigma, R) = \langle x_{\alpha}(a) \mid a\in R,\ \alpha\in \Sigma \rangle \leq \St(\Phi, R)\] is injective. Morever, for any chosen linear order on \(\Sigma\) the map $R^{\Sigma} \to \mathrm{U}(\Sigma, R)$ given by $(r_\alpha)_\alpha \mapsto \prod_\alpha x_\alpha(r_\alpha)$ is a bijection. There is the obvious pro-group analogue \(\mathrm U^{(\infty)}(\Sigma, R)\) with a morphism \(\mathrm U^{(\infty)}(\Sigma, R) \to \St^{(\infty)}(\Phi, R)\) of pro-groups (it seems to be a monomorhism, though we do not need this property) and an isomorphism \(\mathrm U^{(\infty)}(\Sigma, R) \cong (R^{(\infty)})^\Sigma\) of pro-sets. \end{rem}

\begin{rem}
 The definition of the group $\GG^{(s)}(\Phi, R)$ can be reformulated in terms of Hopf algebras. Denote by \(H_\Phi\) be the Hopf \(\ZZ\)-algebra corresponding to the group scheme \(\GG_{\mathrm{sc}}(\Phi, -)\).
...
Our first goal is to show that \(\GG_{\mathrm{sc}}(\Phi, S^{-1} R)\) can be made to act on the Chevalley pro-group \(\GG^{(\infty)}(\Phi, R)\) by conjugation. Denote by \(H_\Phi\) the Hopf \(\ZZ\)-algebra of \(\GG_{\mathrm{sc}}(\Phi, -)\) with the structure of a Hopf algebra given by the triple $(\Delta, S, \varepsilon)$. Recall that \(H_\Phi\) is a finitely presented commutative \(\ZZ\)-algebra, therefore $\Ker(\varepsilon)$ is at least a finitely generated rng. Consider the ``coconjugation'' homomorphism (written down in Sweedler notation)
...
Clearly, this construction gives a pre-morphism \(G^{(\infty)}(\Phi, R) \to G^{(\infty)}(\Phi, R)\) of pro-groups. Up to equivalence, it is independent on the choice of \(s_0\). Finally, this conjugation morphism is functorial on \(g\): for \(g = \varepsilon \colon H_\Phi \to S^{-1} R\) we get \(\mathrm{Coconj}_g = \mathrm{id}\), and for \(g = g_1 * g_2\) the homomorphism \(\mathrm{Coconj}_g\) is the composition
\[\xymatrix{
\Ker(\varepsilon) \ar[r]^(.35){\mathrm{Coconj}_{g_1}} &
S^{-1} R \otimes \Ker(\varepsilon) \ar[r]^(0.35){\mathrm{id} \otimes \mathrm{Coconj}_{g_2}} &
S^{-1}R \otimes S^{-1} R \otimes \Ker(\varepsilon) \ar[r]^(0.55){m \otimes \mathrm{id}} &
S^{-1} R \otimes \Ker(\varepsilon),
}\]
where \(m \colon S^{-1} R \otimes S^{-1} R \to S^{-1} R\) is the multiplication. It is easy to see that \(\up g{(-)} = \up{g_1}(\up{g_2}(-))\) in the category of pro-groups.
...
\end{prop}
\begin{proof} 
 Let us check that the root subgroup morphisms \[\{\up{x_\alpha(u)} x_\beta \colon R^{(\infty)} \to \St^{(\infty)}(\Phi\setminus\{\pm\alpha\}, R) \mid \beta \neq \pm \alpha \}\] defined above satisfy the defining relations of $\St^{(\infty)}(\Phi\setminus\{\pm \alpha\}, R)$. Take roots \(\beta, \gamma \in \Phi \setminus \{\pm \alpha\}\) such that \(\{\alpha, -\alpha\} \cap \{\beta + \gamma, 2\beta + \gamma, \beta + 2\gamma\} = \varnothing\). Then there is a special root subset \(\Sigma \subseteq \Phi\) containing all the roots \(\alpha, \beta, \gamma\), and the required relations are some identities in the pro-group \(\mathrm U^{(\infty)}(\Sigma, R) \cong (R^{(\infty)})^\Sigma\) (see \cref{rem:uni-rad}). But then these identities follows from the ``universal'' identity
 \[[\up{x_\alpha(t)}{x_\beta(b)}, \up{x_\alpha(t)}{x_\gamma(c)}] = \up{x_\alpha(t)}{[x_\beta(b), x_\gamma(c)]}\]
 in \(\St(\Phi, \mathbb Z[b, c, t, t^{-1}])\), the definitions, and the Steinberg relations.
 ...
 %The equivariance of the morphism \(\mathrm{st}\) is clear from the definitions.
 In order to show the equivariance, let \(\mathrm{Cot}_\alpha \colon H_\Phi \to \mathbb Z[X]\) be the homomorphism of Hopf algebras corresponding to \(t_\alpha \colon R \to \GG_{\mathrm{sc}}(\Phi, R)\). The morphism \(\mathrm{st} \circ x_\alpha \colon R^{(s)} \to \GG^{(s)}(\Phi, R)\) is also induced by \(\mathrm{Cot}_\alpha\). By \cref{R3} there is a commutative diagram
 \[\xymatrix{
 \Ker(\varepsilon) \ar[r]^(.4){\mathrm{Coconj}} \ar[d]^{\Delta} &
 H_\Phi \otimes \Ker(\varepsilon) \ar[r]^(0.45){\mathrm{Cot}_\alpha \otimes \mathrm{Cot}_\beta} &
 ...
 Y \mathbb Z[Y] \otimes Z \mathbb Z[Z] \ar[u]_{\substack{Y \mapsto 1 \otimes Y\\Z \mapsto N_{\alpha, \beta} X \otimes Y}} &
 }\]
 for \(\alpha + \beta \in \Phi\) and \(\alpha + 2\beta, 2\alpha + \beta \notin \Phi\). It follows that \(\mathrm{st}(\up{x_\alpha(u)}{x_\beta(b^{(\infty)})}) = \up{t_\alpha(u)}{\mathrm{st}(x_\beta(b^{(\infty)})}\) for such roots. The same argument works for the other three identities from \cref{root-action}. Hence by \cref{SteinbergPresentation} the morphism \(\mathrm{st} \colon \St^{(\infty)}(\Phi, S^{-1} R) \to \GG^{(\infty)}(\Phi, R)\) is \(\GG_{\mathrm{sc}}(\Phi, S^{-1} R)\)-equivariant.
...
In this section we prove the main results of the paper, namely the centrality of $\mathrm{K}_2$.
Actually, we prove a somewhat stronger result, namely we endow the canonical homomorphism $\mathrm{st}\colon \St(\Phi, R) \to \GG_{\mathrm{sc}}(\Phi, R)$ with the structure of a crossed module. Let us briefly recall the definition of the latter. 
...
It is an easy exercise that the kernel of $\varphi$ is a central subgroup of $M$, while the image of $\varphi$ is normal in $N$.
...
\end{theorem}
\begin{proof}  
 Notice that under our assumptions the group $\St(\Phi, R)$ is perfect, therefore by~\cite[Lemma~1.1]{St71} for every $g\in \GG_{\mathrm{sc}}(\Phi, R)$ there may exist at most one endomorphism $\up g (-)$ of $\St(\Phi, R)$ satisfying 
 \begin{equation} \label{eq:scm-2}\up{g}{\mathrm{st}(h)} = \mathrm{st} (g h g^{-1})\text{ for all }h \in \St(\Phi, R).  \end{equation}
 ...
 Indeed, the idea is to reduce to the case of domains using the following result.
 ...
 \begin{proof} Under our assumptions the elementary subgroup $\E_{\mathrm{sc}}(\Phi, R)$ is perfect, therefore one can repeat the proof of \cite[Lemma~5.3]{St99}) verbatim. 
 One will also need to refer to~\cite{Va86} for the commutator formula in the context of Chevalley groups. \end{proof}
