...
\begin{equation} \label{eq:bis}\xymatrix{ \{y_i\} \ar@<3.5pt>[r]^(0.4){\{b_i\}} \ar@<-3.5pt>[r]_(0.4){\{b_i'\}} \ar[d] & \tfrac R {s_1} \otimes K \ar[rd] \ar[rr]^{s_2\otimes \mathrm{id}} & & \tfrac R {s_1 s_2} \otimes K \ar[ld] \\ K \ar[rr]^(.4){(g\otimes \mathrm{id})\circ \mathrm{Coconj}} & & S^{-1} R \otimes K. } \end{equation}
...
Recall from~\cref{sec:group-objects} that it is enough to check that it is morphism of group objects in \(\Pro(\Set)\). By a standard result on Hopf algebras and using that $K$ is flat, we may choose the generators of \(K\) in such a way that $Y = \bigoplus_i \mathbb Z y_i \leq K$ is a finite free submodule and $\Delta(Y) \leq Y \otimeshat Y$. Hence there is a well-defined group homomorphism $Y \to \frac R{s_1} \otimes K, y_i \mapsto b_i$. Take sufficiently large $s_2 \in S$, then the diagram
\[\xymatrix@R=48pt@C=72pt@!0{
Y \otimeshat Y \ar[d] & Y \ar[r] \ar[l]_{\Delta} & \frac R{s_1} \otimes K \ar[d]^{\mathrm{can} \otimes \Delta} \\
(\frac R{s_1} \otimes K)^{\otimeshat 2} \ar[rr] \ar[d]^{(\mathrm{id} \otimes h_1) \otimeshat (\mathrm{id} \otimes h_2)} && \frac R{s_1 s_2} \otimes K^{\otimeshat 2} \ar[d]^{\mathrm{id} \otimes (h_1 \otimeshat h_2)} \\
(R^{(ss_1)})^{\otimeshat 2} \ar[d] && \frac R{s_1 s_2} \otimes (R^{(ss_1 s_2)})^{\otimeshat 2} \ar[d] \\
R^{(ss_1)} \ar[r] & R^{(s)} & \frac R{s_1 s_2} \otimes R^{(ss_1 s_2)} \ar[l]
}\]
is commutative for all rng homomorphisms $h_1, h_2 \colon K \to R^{(ss_1 s_2)}$. Here the unlabelled arrows are the obvious maps constructed by various multiplication homomorphisms and the structure maps. The top face expresses the identity $\up g{(xy)} = \up gx\, \up gy$ in the language of Hopf algebras and the outer face gives the required identity.
