...
For the sake of brevity let us rename the root subgroup morphisms $x_\beta$ of $\St(\Phi\setminus\{\pm\alpha\}, R^{(\infty)})$ for $\beta\neq \pm\alpha$ to $x'_{\beta}$. For the morphisms $x_\beta$ of $\St^{(\infty)}(\Phi, R)$ we continue to use the usual notation. Also denote $\St^{(\infty)}(\Phi, R)$ by $H^{(\infty)}$ and $\St^{(\infty)}(\Phi\setminus\{\pm\alpha\}, R)$ by $H^{(\infty)}_\alpha$.

By~\cref{SteinbergPresentation} the morphisms $\widetilde{x}_{\pm\alpha}$ together with $x'_\beta$, $\beta\neq \pm \alpha$ give rise to a morphism $G_\alpha \colon H^{(\infty)} \to H^{(\infty)}_\alpha$. By definition, $G_\alpha x_\beta = x_\beta'$ for $\beta\neq\pm\alpha$ and $G_\alpha x_{\pm\alpha} = \widetilde x_{\pm\alpha}$. On the other hand, it is clear that $F_\alpha x_\beta' = x_\beta$ for $\beta\neq \pm\alpha$. To see that $G_\alpha$ is inverse to $F_\alpha$ it remains to use the first part of~\cref{SteinbergPresentation} and~\cref{rem:xpma-presentation}.
\end{proof}
