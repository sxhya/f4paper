...
The category of pro-sets \(\Pro(\Set)\) has all finite limits by \cite[Prop.~6.1.18]{SK06} and therefore is a symmetric monoidal category.
...
\begin{rem}\label{rem:prorings-comment}
 ...
 This also shows that the set of (global) elements of $R^{(\infty)}$ (i.\,e. the hom-set in $\Pro(\Set)$ from the terminal object $1$ to $R^{(\infty)}$) is often trivial and therefore will be of little interest to us.
\end{rem}
...
\begin{df} \label{df-pro-set-morphisms}
 Let $R^{(\infty)}$ be an rng object in $\Pro(\Set)$ (e.\,g. a pro-rng) and $p \in \mathbb{Z}[t_1,\ldots t_n]$ be polynomial with integer coefficients.
 We denote by $p^{(\infty)}$ the morphism $(R^{(\infty)})^{n} \to R^{(\infty)}$ ``modeling'' $p$ in the category of pro-sets.
 This morphism can be obtained as an appropriate composition of the following morphisms:
 \begin{itemize}
  \item diagonal morphisms $\Delta_{R^{(\infty)}}$ and projections $\pi_i$ (which are part of the structure of a symmetric monoidal category on $\Pro(\Set)$);
  \item the addition and multiplication morphisms $+_{R^{(\infty)}}, m_{R^{(\infty)}}$ defining the structure of an rng-object on $R^{(\infty)}$.
 \end{itemize}
 
 Now if $G^{(\infty)}$ is a group object in $\Pro(\Set)$ (e.\,g. a pro-group) and $w$ a word of the free group $F(t_1, \ldots, t_n)$ we denote by $w^{(\infty)}$ the pro-set morphism $(G^{(\infty)})^{n} \to G^{(\infty)}$ modeling $w$ in the category of pro-sets. As before, this morphism can be defined as an appropriate composition of morphisms $m_{G^{(\infty)}}$, $i_{G^{(\infty)}}$, $e_{G^{(\infty)}}$ defining the structure of a group object on $G^{(\infty)}$ and the morphisms defining the structure of a symmetric monoidal category on $\Pro(\Set)$.
\end{df}

\begin{example}\label{example-commutator}
 In the case $w = [t_1, t_2] = t_1 t_2 t_1^{-1} t_2^{-1} \in F[t_1, t_2]$ the associated morphism $w^{(\infty)}$ can be defined as the composition
 \[ \xymatrix{G^{(\infty)} \times G^{(\infty)} \ar[r]^(.35){\Delta \times \Delta} & G^{(\infty)} \times G^{(\infty)} \times G^{(\infty)} \times G^{(\infty)} \ar[d]_{\begin{psmallmatrix} \id & 0 & 0 & 0 \\ 0 & 0 & \id & 0 \\ 0 & i & 0 & 0 \\ 0 & 0 & 0 & i \end{psmallmatrix}} & \\
    & G^{(\infty)} \times G^{(\infty)} \times G^{(\infty)} \times G^{(\infty)} \ar[r]^(.76){m(m\times m)} & G^{(\infty)}.} \]
\end{example}
In order simplify our notation even further we employ the following conventions:
 \begin{itemize}
  \item any algebraic expression containing the symbol $(\infty)$ in the upper index of a variable (e.\,g. $a^{(\infty)}$, $b^{(\infty)}$ etc.)  denotes a certain morphism of pro-sets, whose domain is a certain product of pro-sets (often a power of a single pro-set);
  \item the number of factors in this product coincides with the number of different variables occuring in the expression;
  \item in order to specify the domain of a variable $a^{(\infty)}$ occuring in the expression, we use the notation $a^{(\infty)} \in X^{(\infty)}$ (this notation is not confusing since we never refer to the actual elements of pro-sets, see~\cref{rem:prorings-comment});
  \item to denote the pro-set morphism corresponding to a word of a free group or a polynomial with integer coefficients (see~\cref{df-pro-set-morphisms}) we use the same word (resp. polynomial) but add $(\infty)$ symbol to the upper indices of all occurrences of variables (e.\,g. with these notations the morphism $w^{(\infty)}$ from~\cref{example-commutator} is denoted $[t_1^{(\infty)}, t_2^{(\infty)}]$);
