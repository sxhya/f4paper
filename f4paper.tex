...
\title{Centrality of $K_2$ for Chevalley groups: a pro-group approach}
...
\author {Egor Voronetsky} \email {voronetckiiegor at yandex.ru}
...
For an infinite field $F$ the Steinberg group $\mathrm{St}(\Phi,\,F)$ no longer coincides with the simply-connected Chevalley group $\GG_{sc}(\Phi, F)$, but rather is the universal central extension of the latter.
By the classical theory of central extensions the kernel of the natural homomorphism $\St(\Phi,\,F)\to\GG_{sc}(\Phi, F)$ coincides with the Schur multiplier $\mathrm H_2(\mathrm G_{\mathrm{sc}}(\Phi,\,F),\,\mathbb Z)$. The concrete presentation of this kernel in the field case was obtained by H.~Matsumoto in~\cite{Ma69}. Similar presentation for the special linear group over a skew field also has been obtained by U.~Rehmann in~\cite{Re78}.
...
On the other hand, the analogue of Suslin's normality theorem for Chevalley groups of rank $\geq 2$, i.\,e. the normality of the subgroup $\E_{sc}(\Phi, R):= \mathrm{Im}(\mathrm{st})$ for an arbitrary commutative ring $R$, was soon obtained by G.~Taddei, see~\cite{Ta86}. 
...
\[ \xymatrix{ 1\ar@{->}[r]&{\mathrm H_2(\mathrm{St}(\Phi,\,R),\mathbb Z)}\ar@{->}[r]&\mathrm K_2^Q(\Phi,\,R)\ar@{->}[r]&\mathrm K_2(\Phi,\,R)\ar@{->}[r]&1,} \text{where} \]
\begin{align*}
...
By the centrality of $\mathrm K_{2,n}(R_M)$ one has $[\lambda_{M}(g),\,x_{ij}(r)]=1$ for all $r\in R_{M}$ (we denote by $\lambda_M$ the localization homomorphism $R \to R_M$). If we were able to show for some $s_0\in R\setminus M$ that
\begin{equation} \label{intro} [g,\,x_{ij}(s_0R)]=1, \end{equation}
or, in other words, that $s_0\in I\cap R\setminus M$, this would lead to a contradiction and would complete the proof of the centrality of $\mathrm K_{2,n}(R)$.

Now take any presentation of $g$ as $\prod_hx_{k_h l_h}(r_h)$ for some $r_h\in R$ and $k_h\neq l_h$. The equality $[\prod_hx_{k_h l_h}(r_h),\,x_{ij}(s_0r)]=1$ in $\St_n(R_M)$ is a consequence of a finite number of Steinberg relations involving denominators from $R\setminus M$. Naively one would hope to ``get rid of'' these denominators, i.\,e. to find $s_0 \in R\setminus M$ so large that the relation $[g,\,x_{ij}(s_0r)]=1$ becomes a consequence of Steinberg relations defining the group $\St_n(R)$.

Unfortunately, formally proving the last claim is not easy. Clasically, the proof went as follows (cf.~\cite{Tul,Sin,LavSin,Lavlgp}): first, the localization at a maximal ideal was reduced to the localization at some element $s$ (cf. e.\,g. the proof of~\cite[Theorem~2]{Sin}), then a trick called {\it Dilation principle} was used (cf. e.\,g.~\cite[Lemma~15]{Sin}).
The proof of the Dilation principle, however, further required finding nontrivial ad hoc presentations of relative Steinberg groups.
For example~\cite{Tul, Lavlgp} relied on a variant of van der Kallen's another presentation for linear and symplectic groups, while~\cite{Sin, LavSin} relied on the amalgamation theorem for relative Steinberg groups; see~\cite[Theorem~9]{Sin}. However, it was not easy to transfer these approaches to all Chevalley groups.

A remarkable idea of the third-named author was to abandon relative Steinberg groups $\St(\Phi, R, I)$ and instead work with sequences of {\it unrelativized} Steinberg groups $\St(\Phi, s^nR)$, $n \in \mathbb N$ given by the usual Steinberg presentations.
It turns out that it is possible to define the ``conjugation'' action of the Steinberg group $\St(\Phi, R_s)$ upon such a sequence. Naively, for each $g \in \St(\Phi, R_s)$ this automorphism of conjugation by $g$ corresponds to a collection of group homomorphisms \[\{\St(\Phi, s^{m_2}R) \to \St(\Phi, s^{m_1}R)\}_{m_1 \in \mathbb N },\] where $m_2$ depends on $g$ and $m_1$.
...
The above diagram is called the {\it Steinberg pro-group}. It is possible to define an action of $\St(\Phi, S^{-1}R)$ upon the Steinberg pro-group. For each $x_{\alpha}(a/s)$, $a\in R$, $s\in S=R\setminus M$ the automorphism of conjugation by $x_\alpha(a/s)$ corresponds to a collection of group homomorphisms \[\{\St(\Phi, f(s)R) \to \St(\Phi, sR)\}_{s \in S},\] where $f \colon S \to S$ is some function such that $s$ divides $f(s)$. In fact, a morphism between pro-groups should be defined not just as a collection of homomorphisms, but as an {\it equivalence class} of such collections. We refer the reader to~\cref{sec:pro-obj} for more formal treatment of pro-groups and to~\cref{sec:local-action} for the formal definition of the action of the Steinberg group $\St(\Phi, S^{-1}R)$.

Now the proof of~\eqref{intro} goes as follows.
...
\begin{rem}
 It is possible to prove the main results of the paper without using ring homotopes.
 This approach is outlined in~\cref{no-homotopes}.
\end{rem}
...
\section{The action of \texorpdfstring{$\GG_{sc}(\Phi, S^{-1}R)$}{G(Ф, R)} on pro-groups} \label{sec:local-action}
...
\begin{proof}
 ...
 Since $R_M$ is local, the group $\GG_{sc}(\Phi, R_M)$ coincides with its elementary subgroup $\E_{sc}(\Phi, R)$.
...
Actually, we prove a somewhat stronger result, namely we endow the canonical homomorphism $\mathrm{st}\colon \St(\Phi, R) \to \GG_{sc}(\Phi, R)$ with the structure of a crossed module. Let us briefly recall the definition of the latter. 
...
\begin{proof}  
 ...
 \begin{proof} Under our assumptions the elementary subgroup $\E_{sc}(\Phi, R)$ is perfect, therefore one can repeat the proof of \cite[Lemma~5.3]{St99}) verbatim. 
