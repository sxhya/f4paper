...
It is clear that the latter commutator is trivial by~\eqref{eq:comm-mult-rhs} and~\eqref{R2-bg}.  
 ...
 \begin{align*}
  ...
  x_{i\alpha - j\beta_1 - k\beta_2}\bigl(B_{i, j, k} \cdot {b^{(\infty)}}^i {c^{(\infty)}}^i {b_1^{(\infty)}}^j {b_2^{(\infty)}}^k\bigr),  
 \end{align*}
for some integers $B_{i,j,k}$.
Direct computation or an argument similar to the one used in the first part of the proof shows that all the constants \(B_{i, j, k}\) are zero with the sole exception of \(B_{1, 1, 1}$, which is equal to $N_{\alpha, -\beta}\). Consequently, by~\cref{RingGeneration} one has
...
\begin{proof}
 ...
 Notice also that~\eqref{eq:comm-mult-lhs} and~\cref{lem:elim-lhs} imply that
 \begin{align*}
 ...
 Thus, the morphism \(x_{\beta, \gamma}\) satisfies the first three conditions of~\cref{RingPresentation}. 
 ...
 Now suppose that \(\alpha = \beta_1 + \gamma_1 = \beta_2 + \gamma_2\) are two different decompositions for $\alpha$ such that $\alpha$, $\beta_i$ and $\gamma_i$ have the same length and \(\alpha\), \(\beta_1\), \(\beta_2\) are linearly independent.
