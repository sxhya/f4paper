...
\end{proof}
 ...
 \begin{theorem}\label{DoubleRootElimination}
  Let \(R\) be a ring, \(S \subseteq R\) be a multiplicative subset, \(\Phi\) be a root system of rank \(\geq 3\) different from \(\rB_\ell\) and \(\rC_\ell\).
  Let \(\Phi_0 \subseteq \Phi\) be a subsystem of rank \(2\) spanned by a pair of nonparallel roots $\alpha, \beta \in \Phi$ (i.e. \(\Phi_0 = \Phi \cap (\mathbb R\alpha + \mathbb R\beta)\)). Then the collection of root subgroup morphisms $x_\gamma \colon R^{(\infty)} \to \St^{(\infty)}(\Phi, R)$ for $\gamma \in \Phi\setminus \Phi_0$ generate $\St^{(\infty)}(\Phi, R)$ in the categorical sense, i.\,e. to show that a pair of morphisms $f_1, f_2 \colon \St^{(\infty)}(\Phi, R) \to G^{(\infty)}$ are equal it suffices to show that $f_1x_\gamma = f_2x_\gamma$ for all $\gamma \in \Phi\setminus\Phi_0$.
 \end{theorem}
 \begin{proof}
...
Our first goal is to show that \(\GG_{sc}(\Phi, S^{-1} R)\) can be made to act on the Chevalley pro-group \(\GG^{(\infty)}(\Phi, R)\) by conjugation. Indeed, denote by \(H_\Phi\) the Hopf \(\ZZ\)-algebra of \(\GG_{sc}(\Phi, -)\) with the structure of a Hopf algebra given by the triple $(\Delta, S, \varepsilon)$. Recall that \(H_\Phi\) is a finitely presented commutative \(\ZZ\)-algebra, therefore $\Ker(\varepsilon)$ is at least a finitely generated rng. Consider the ``co-conjugation'' map (written down in Sweedler notation)
