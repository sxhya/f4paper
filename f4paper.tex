...
It follows that in this case \(\mathrm{st}(\up{x_\alpha(u)}{x_\beta(b^{(\infty)})}) = \up{t_\alpha(u)}{\mathrm{st}(x_\beta(b^{(\infty)})}\). The same argument works for the other three identities from \cref{root-action}. Thus, by \cref{SteinbergPresentation} the morphism \(\mathrm{st} \colon \St^{(\infty)}(\Phi, S^{-1} R) \to \GG^{(\infty)}(\Phi, R)\) is \(\GG_{\mathrm{sc}}(\Phi, S^{-1} R)\)-equivariant.
