...
\end{theorem}
\begin{proof}
 We construct a morphism \(G_\alpha \colon \St^{(\infty)}(\Phi, R) \to \St^{(\infty)}(\Phi / \alpha, R)\) of pro-groups such that \(G_\alpha \circ x_\beta = x_\beta\) for all \(\beta \nparallel \alpha\). It suffice to find morphisms \(\widetilde x_{\pm \alpha} \colon R^{(\infty)} \to \St^{(\infty)}(\Phi / \alpha, R)\) satisfying the Steinberg relations by lemma \ref{SteinbergPresentation}. Since no relation contains both \(\alpha\) and \(-\alpha\), we only have to deal with \(\alpha\). For any pair of roots \((\beta, \gamma)\) such that \(|\beta| = |\gamma| = |\alpha|\) and \(\alpha = \beta + \gamma\) let
 \begin{align*}
