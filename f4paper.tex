...
Since the root elements $x_\alpha^{(s)}(a)$ generate $\St(\Phi, R^{(s)})$ we conclude that the morphisms $f_1$ and $f_2$ are equal.
...
\begin{proof}
 ...
 Notice that the root subset $\Sigma = \Phi \cap (\ZZ_{\geq 0}\beta + \ZZ_{\geq 0}\gamma + \ZZ_{> 0}\delta)$ is special and does not contain $\alpha$. Thus, the relations between the remaining root subgroup morphisms of $\St^{(\infty)}(\Phi\setminus\{\alpha\}, R)$ suffice to rewrite the left-hand side of~\eqref{R2-bg}--\eqref{R4-bg2} as follows:
 \begin{align}
...
We claim that the rng \(T = \mathrm{Coconj}(\Ker(\varepsilon))\) is finitely presented. Indeed, it is finitely generated as the image of a finitely generated rng \(\Ker(\varepsilon)\). Let \(a_1, \ldots, a_n\) be the generators of \(T\), so the ring homomorphism \(\ZZ[X_1, \ldots, X_n] \to T \rtimes \ZZ, X_i \mapsto a_i\) is surjective. Its kernel \(I\) is generated by a finitely many polynomials \(f_1, \ldots, f_m\) by Hilbert's basis theorem. Clearly, \(f_j\) have zero constant terms and \(f_j(a_1, \ldots, a_m) = 0\) are all the relations between \(a_i\). Let \(N\) be the maximal degree of \(f_j\).

Recall that \(S^{-1} R\) is the direct limit of \(R\)-modules \(\frac Rs\) for \(s \in S\), each of these modules is isomorphic to \(R\). We omit the notation for the canonical homomorphisms \(\frac R{s} \to \frac R{ss'}\) and \(\frac Rs \to S^{-1} R\). There is also a canonical isomorphism \(\frac Rs \otimes_R R^{(ss')} \cong R^{(s')}, \frac rs \otimes {r'}^{(ss')} \mapsto (rr')^{(s')}\) for every \(s, s' \in S\), so the \(R\)-module \(\frac Rs \otimes_R R^{(ss')}\) has a natural multiplication.

Let $g$ be an element of $\mathbf{Ring}(H_\Phi, S^{-1}R) \cong \GG_{\mathrm{sc}}(\Phi, S^{-1} R)$ and consider the rng homomorphism \(g \otimes \mathrm{id} \colon T \to S^{-1} R \otimes \Ker(\varepsilon)\). It is easy to see that there is \(s_0 \in S\) such that all \((g \otimes \mathrm{id})(a_i)\) may be lifted to \(\frac R{s_0} \otimes \Ker(\varepsilon)\). Let us choose such elements \(\up g{a_i} \in \frac R{s_0} \otimes \Ker(\varepsilon)\). There also is \(s_1 \in S s_0^N\) such that \(f_j(\up g{a_1}, \ldots, \up g{a_n}) = 0 \in \frac R{s_1} \otimes \Ker(\varepsilon)\) for all \(j\).
...
$\Rng(\Ker(\varepsilon), R^{(f(s))}) \to \Rng(\Ker(\varepsilon), R^{(s)})$ for some function $f \colon S \to S$. Set $f(s) = ss_1$ and let $h$ be an element of $\Rng(\Ker(\varepsilon), R^{(ss_1)})$. It is clear that the homomorphism
\[\textstyle T \to \frac R{s_1} \otimes_R R^{(s s_1)} \cong R^{(s)}, a_i \mapsto (\mathrm{id} \otimes h)(\up g{a_i})\]
is well-defined. The required element \(\up gh\) is its composition with \(\Ker(\varepsilon) \to T\).

Clearly, the resulting morphism of pro-groups is independent on the choices of \(s_0\), \(s_1\), and \(\widetilde u_i\). It easily follows that
\begin{align*}
\up 1{(-)} &= \mathrm{id} \colon \GG^{(\infty)}(\Phi, R) \to \GG^{(\infty)}(\Phi, R),\\
\up{gg'}{(-)} &= \up{g}{(\up{g'}{(-)})} \colon \GG^{(\infty)}(\Phi, R) \to \GG^{(\infty)}(\Phi, R)
\end{align*}
for all \(g, g' \in \GG_{\mathrm{sc}}(\Phi, S^{-1} R)\).
