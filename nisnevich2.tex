...
\begin{document}
\begin{abstract}
 Let $k$ be an arbitrary field. In this paper we show that in the linear case ($\Phi=\rA_\ell$, $\ell \geq 4$) and even orthogonal case ($\Phi = \rD_\ell$, $\ell\geq 7$, $\mathrm{char}(k)\neq 2$) the unstable functor $\mathrm{K}_2(\Phi, -)$ possesses the $\mathbb{A}^1$-invariance property in the geometric case, i.\,e. $\K_2(\Phi, R[t]) = \K_2(\Phi, R)$ for regular rings $R$ containing $k$. As a consequence, the unstable $\K_2$ groups can be represented in the unstable $\mathbb{A}^1$-homotopy category $\mathcal{H}^{\mathbb{A}^1}_{k}$ as fundamental groups of the simply-connected Chevalley--Demazure group schemes $\mathrm{G}(\Phi,-)$. Our invariance result can be considered as the $\K_2$-analogue of the geometric case of Bass--Quillen conjecture. We also show for a semilocal regular $k$-algebra $A$ that $\K_2(\Phi, A)$ embeds as a subgroup into $\K^\mathrm{M}_2(\mathrm{Frac}(A))$.
\end{abstract}

\maketitle

\section{Introduction}
%In this paper we prove the $\mathbb{A}^1$-invariance for the unstable linear and orthogonal $\K_2$-functors.
%As a consequence, we obtain that several variants of the definition of the $\K_2$-functor associated to linear and orthogonal Chevalley groups lead to the same answer.

Historically there have been many different approaches to higher algebraic $\K$-theory and Hermitian $\K$-theory.
Certain approaches suit better for the purposes of theory-building while others are more amenable to computation. 
Sometimes these approaches lead to the same answer. Recall, for example, from~\cite[Theorem~IV.11.8]{Kbook} that stable Quillen's groups $\K_n(R)$ (defined e.\,g. via the $+$-construction) and stable Karoubi--Villamayor groups $\mathrm{KV}_n(R)$ coincide for $n\geq 1$ if $R$ happens to be a regular ring.
Notice that both of these theories use infinite-dimensional algebraic groups (such as $\mathrm{GL}_\infty(R)$ or $\mathrm{O}_\infty(R)$, $\mathrm{Sp}_\infty(R)$) in their definition.

One can define and study the analogues of these $\K$-groups for finite-dimensional algebraic groups, see e.\,g.~\cite{St71, Re75, St78, Tu83, Abe83, Sta14, VW16, Sta20}. The interest for the unstable $\K_2$-groups comes, in particular, from the fact that they appear in Steinberg's presentation of (the groups of points of) algebraic groups by means of generators and relations. Let us recall this presentation in greater detail. Let $R$ be a commutative ring with a unit and $\Phi$ be an irreducible root system of rank $\geq 2$. The Steinberg group $\St(\Phi, R)$ is a certain group defined by means of generators and relations, see~\cite[Ch.~6]{St67} (see also~\cref{sec:main-results}).
...
Notice that the groups $\K_i(\Phi, R)$ from~\cite{St78} are precisely the groups $\K_i^G(R)$ in the case when $G$ is the simply-connected form of the Chevalley group, i.\,e. $\Lambda = P(\Phi)$ (for shortness we also write $\Gsc(\Phi,-)$ instead of $\mathrm{G}_{P(\Phi)}(\Phi, R)$). In many cases (e.\,g. when $R$ is a ring of stable rank $1$ or a Dedekind domain of arithmetic type) the term $\K_1(\Phi, R)$ in the right-hand side of the above sequence vanishes and $\Gsc(\Phi, R)$ becomes the quotient of $\St(\Phi, R)$ by $\K_2(\Phi, R)$. Thus, the group $\K_2^G(R)$ can interpreted as the group of ``nontrivial'' relations between $x_\alpha(\xi)$ (the ``trivial'' relations being Chevalley commutator formulas~\eqref{R1}--\eqref{R3}). Our first result is the following
\begin{theorem}[The $\K_2$-analogue of Lindel--Popescu theorem] \label{theorem:LP-for-K2}
Let $k$ be an arbitrary field and $R$ be a regular ring containing $k$. Let $\Phi$ be either $\rA_\ell$ for $\ell\geq4$, or $\rD_\ell$ for $\ell\geq 7$. In the latter case assume additionally that the characteristic of $k$ is not $2$. Then for any $\Lambda$ and $G = \mathrm{G}_\Lambda(\Phi, -)$ one has $\K_2^G(R[t])\cong\K_2^G(R).$
\end{theorem} 
...
Combining~\cref{theorem:LP-for-K2} with~\eqref{eq:H2-K2} we obtain yet another corollaries of~\cref{theorem:LP-for-K2}:
\begin{corollary}
