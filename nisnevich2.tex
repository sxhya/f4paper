...
\begin{proof}[Proof of~\cref{cor:various-facts}]
 ...
 \begin{proof}[Proof of~\cref{cor:H_2-O}]
  ...
  The functor $\mathrm H_2(\mathrm{EO}_{2\ell}(-))$ clearly coincides with $\mathrm{KSO}_{2, 2\ell} := \K_2^{\mathrm {SO}_{2\ell}}$ and is also $\mathbb{A}^1$-invariant by~\cref{theorem:LP-for-K2}. Thus, by 5-lemma we conclude that the central term is also $\mathbb{A}^1$-invariant.
 \end{proof}
