...
\begin{corollary}
For $k$ with $\mathrm{char}(k)\neq 2$ one has $\mathrm H_2(\mathrm{Spin}_{2l}(k[t_1,\ldots,t_n]),\,\mathbb Z\big) = \K^\mathrm{M}_2(k)$.
\end{corollary}
...
Now the assertion of~\cref{theorem:LP-for-K2} follows from~\cref{lpb}, while the assertion of~\cref{theorem:Gersten} follows from~\cref{thm:gb}.
\end{proof}
...
We refer the reader to \cite[Section~6.1]{SK06} for a introduction to the formalism of ind-objects and pro-objects. We also refer to~\cite[\S~2]{LSV20} for a more detailed exposition of the material presented in this subsection.
