...
Notice that $R_a$ is the zero rng if $a$ is nilpotent. It will be convenient for us to extend the domain of definition for $K$ and $NK$ so that it includes the zero rng. We do this by setting $K(0) = NK(0) = 1$.

For $R \in \mathcal{R}$ and $a \in R$ consider the following commutative square:
...
Following Swan we say that a ring $R$ containing a field $F$ is {\it geometrically regular} over $F$, if for any finite field extension $E$ of $F$ the ring $R\otimes_FE$ is regular (cf.~\cite[p.~137]{Sw98}). In particular, such a ring is noetherian. If $F$ is perfect then a ring $R$ is geometrically regular over $F$ if and only if it is regular and contains $F$.
...
\begin{theorem} \label{lpb}
 ...
 \begin{enumerate}[label=\textnormal{(A\arabic*)}]
  ...
  For every $R \in \mathcal{R}$ and $a \in R$ consider the diagram obtained from~\eqref{M-sq} by applying $K$. Then the homomorphism
  \begin{equation}\label{eq:DP-map} \Ker(K(e)) \to NK(R_a)\end{equation}
  between the kernels of vertical arrows is injective.
 ...
 \end{enumerate}
 Then for every ring $R\in \mathcal{R}$ geometrically regular over a field $F \in \mathcal{R}$ the natural embedding $R \hookrightarrow R[t]$ induces an isomorphism $K(R)\cong K(R[t]).$
\end{theorem}
\begin{rem}
 ...
 Our axiom~\eqref{LPP} is analogous to the axiom (P3) from~\cite[Proposition~3.3.4]{AHW18}.
\end{rem}
...
\begin{lemma}\label{LGP}
 Suppose that $K$ is finitary and satisfies the axiom~\ref{DP}. Then $K$ satisfies the Quillen--Suslin local-global principle. In particular, $NK$ is a separated presheaf in the Zariski topology.
\end{lemma}
...
\begin{lemma}[weak affine Zariski excision for domains]
	\label{zgl} Suppose that $K$ satisfies the axiom~\ref{LPP}. Let $R\in \mathcal{R}$ be a domain and $a, b$ be a pair of coprime elements of $R$. Consider the diagram
$$\begin{tikzcd}
...
Suppose that $K$ satisfies weak affine Zariski excision and $\mathbb{P}^1$-glueing properties for domains. Then for any domain $R\in \mathcal{R}$ and any monic polynomial $f\in R[t]$ the localization homomorphism $\lambda_f\colon R[t]\rightarrow R[t]_f$ induces an injection $K(R[t])\hookrightarrow K(R[t]_f).$ \end{lemma}
\begin{proof}
...
Let $F$ be a field. Recall that an $F$-algebra $R$ is called {\it essentially smooth} if it is geometrically regular and essentially of finite type over $F$. 
Equivalently, $R$ is essentially smooth over $F$ if it is essentially of finite type over $F$ and $R\otimes_F\overline F$ is regular (see e.\,g.~\cite[p.~137]{Sw98}). 
If, moreover, $R$ is of finite type over $F$, it is called {\it smooth over $F$}.
...
Recall the following geometric result.
\begin{theorem}[Panin]
\label{paninthm}
Let $F$ be a field, $R$ be a domain smooth over $F$. Let $M_1,\ldots, M_n$ be a finite set of maximal ideals of $R$. Denote by $A=R_{M_1,\ldots,M_n}$ the correponding semi-localization of $R$. Let $f$ be an element of the intersection  $\cap_{i=1}^nM_i \subseteq R$. Then there exists a monic polynomial $h(t)\in A[t]$, a domain $S$ essentially smooth over $F$ and homomorphisms $\tau$, $p$, $p'$ and $\delta$ such that $A[t]/hA[t]\cong S/\tau(h)S$ and the following diagram commutes:
\begin{equation*}
...
\begin{corollary}
\label{esssmooth}
Suppose that $K\colon\mathcal{R}\rightarrow\catname{Grp}$ is finitary and satisfies weak affine Nisnevich excision and $\mathbb{P}^1$-glueing properties for domains. Let $F\in\mathcal R$ be a field, and $R, M_1, \ldots M_n, A$ be as in the statement of the above theorem. Denote by $E$ the fraction field of $A$ {\rm(}which coincides with the fraction field of $R${\rm)}.
Then for any $m\geq0$ the homomorphism $K(A[x_1,\ldots, x_m])\to K(E[x_1,\ldots, x_m])$ is injective.
\end{corollary}
\begin{proof}
Let $g$ be an element of the kernel of $K(A[x_1,\ldots, x_m])\rightarrow K(E[x_1,\ldots, x_m])$. Since $K$ commutes with filtered colimits, there exists a smooth $F$-algebra $R'$, maximal ideals $M_i'$ in it, and $f\in\cap M_i'\subseteq R'$ such that $A$ is the semi-localization of $R'$ in $M_i'$, and
$g=K(\lambda_{M_1',\ldots,M_n'})(g_1)$
for some $g_1\in\mathrm{Ker}\left(\mathrm K(R'[x_1,\ldots, x_m])\rightarrow\mathrm K(R_f'[x_1,\ldots, x_m])\right).$ 
In fact, one can take $R'$  to be a principal localization of $R$.

We will apply Theorem~\ref{paninthm} to the ring $R'$, maximal ideals $M_i'$ and $f$ chosen above. We will denote the induced maps on polynomial rings by the same letters. Consider the diagram
\begin{equation*} 
 \begin{tikzcd}[column sep=4em]
   & K(A[x_1,\ldots, x_m]) & \\ 
   K(A[x_1,\ldots, x_m][t]) \ar{d}{\lambda_h} \ar{ru}{ev_t(0)} \ar{r}{\tau} & K(S[x_1,\ldots, x_m]) \ar{u}{\delta} \ar{d}{\lambda_{\tau(h)}}  & K(R'[x_1,\ldots, x_m]) \ar{ul}[swap]{\lambda} \ar{d}{\lambda_f} \ar{l}[swap]{p} \\
   K(A[t][x_1,\ldots, x_m]_h) \ar{r}[swap]{}              & K(S[x_1,\ldots, x_m]_{\tau(h)})      & K(R'[x_1,\ldots, x_m]_f). \ar{l}{}\end{tikzcd}
\end{equation*}
Observe that $p(g_1)$ is mapped to $1$ via $\lambda_{\tau(h)}$, and therefore by~\ref{LPP} there exist $g_2\in\mathrm K(A[x_1,\ldots, x_m][t])$ such that $\lambda_h(g_2)=1$ and $\tau(g_2)=p(g_1)$. Since $h$ is monic, we conclude by~\cref{lmp} that the homomorphism $ K(A[x_1,\ldots, x_m][t])\to K(A[x_1,\ldots, x_m][t]_h)$ is injective and consequently that $g_2=1$. It remains to observe that
$$g=\lambda_{}(g_1)=\delta(p(g_1))=\delta(\tau(g_2))=g_2(0)=1.\qedhere$$
\end{proof}
\begin{rem}
In fact we will use only the case $m=1$ in the proof below, and after~\cref{lpb} is proven, the statement for any $m>0$ follows from the case $m=0$.
\end{rem}

Now we can use the results of Popescu to prove~\cref{lpb}. 

\begin{theorem}[Popescu]
\label{popescu}
Let $F$ be a field, and $R$ a noetherian ring geometrically regular over $F$. Then $R$ is a filtred colimit of smooth $F$-algebras. If, moreover, $R$ is local, then $R$ is a filtered colimit of essentially smooth local $F$-algebras.
\end{theorem}
\begin{proof} See~\cite{Po90} and also~\cite[Theorem~1.1, Corollary~1.3]{Sw98}.%, see also~\cite{Po90}.
\end{proof}

Now we are ready to finish the proof of \cref{lpb}. %Our argument follows~\cite{Sta14, Vo81}.
\begin{proof}[Proof of~\cref{lpb}]
Thanks to~\cref{LGP}, we may assume additionally that $R$ is local, in particular, a domain. 
%Since every field contains a perfect subfield we may assume that $F$ is perfect.
Since $K$ commutes with filtered colimits we conclude from~\cref{popescu} that it suffices to verify the assertion of the theorem for an essentially smooth local $F$-algebra $R$.  %
...
$$\begin{tikzcd}
K\bigl(R[t]\bigr) \ar[hookrightarrow]{r} \ar{d}{\mathrm{ev}_t(0)} & K\bigl((\mathrm{Frac}\,R)[t]\bigr) \ar{d}{\cong}\\
K\bigl(R\bigr) \ar[hookrightarrow]{r} & K\bigl(\mathrm{Frac}\,R\bigr),
\end{tikzcd}$$
where the horisontal arrows are injective by Corollary~\ref{esssmooth}, and the right vertical arrow is an isomorphis by~\ref{HIF}. Now the injectivity of the left vertical arrow follows from the diagram (and the surjectivity is trivial).
...
Let $A$ be a semi-local noetherian domain containing a field $F\in\mathcal R$, and assume that $A$ is geometrically regular over $F$. 
Denote by $E$ the fraction field of $A$.
Then the natural homomorphism $K(A) \to K(E)$ is injective.
\end{theorem}
\begin{proof}
By Theorem~\ref{popescu} we know that $A$ is a filtered colimit of smooth $F$-algebras. Since $A$ is a domain we may require additionally these smooth $F$-algebras to be domains.
...
Consider the preimages $P_i$ of maximal ideals of $A$, and take some maximal ideals $M_i$ of $R$ that contain $P_i$. Then by \cref{esssmooth}, $g_1$ vanishes in the semi-localisation of $R$ in a finite set of maximal ideals $M_i$, therefore it vanishes in the semi-localisation of $R$ in a set of prime ideals $P_i$. But $R\rightarrow A$ factors through this semi-localisation, so we conclude that $g=1$. 
\end{proof}


We remark, that an analogous theorem for $\mathrm K_1^G$-functors is proven by A.~Stavrova in~\cite[Theorem~1.2]{Sta20}

%TODO: Процитировать статью НАсти "equicharacteristic", Proof of Theorem~1.1.
