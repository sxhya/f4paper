...
The proof of the fact $T_\alpha$ satisfy relations~\eqref{Chevalley-CCF2} is almost identical to the above proof, so we leave it as an exercise to the reader. %$x_\alpha(\frac c {h^s})\, x_\beta(\frac{c'}{h^s})$ and $x_\beta(\frac{c'}{h^s})\, x_\alpha(\frac c {h^s})$ identically act for $\alpha + \beta \notin \Phi \cup \{0\}$ is the same.
