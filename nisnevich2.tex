...
\begin{rem}
In the sequel we, in fact, only need the special cases $B=k$, $B=k[t]$ of the above result. 
\end{rem}
...
\end{proof}
\begin{rem}
It is clear from the proof above that it possible to weaken the requirements on $K$ in the statement of~\cref{lpb}. For example, we could require that the assertions of the axioms \ref{DP}--\ref{PGP} hold true only for those $k$-algebras which are essentially smooth over $k$.
\end{rem}
