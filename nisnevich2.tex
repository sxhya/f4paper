...
[x_{\alpha}(a),\,x_{\beta}(b)]  &=1,\text{ for }\alpha+\beta\not\in\Phi\cup; \tag{R2} \label{R2} \\
[x_{\alpha}(a),\,x_{\beta}(b)]  &=x_{\alpha+\beta}(N_{\alpha\beta} \cdot ab)\text{ in the case }\alpha+\beta\in\Phi,\text{ but } \alpha+2\beta,\ 2\alpha+\beta\not\in\Phi; \tag{R3} \label{R3} \\
[x_\alpha(a),\ x_\beta(b)] &= x_{\alpha + \beta}(N_{\alpha,\beta} \cdot ab) \cdot x_{2\alpha+\beta}(N_{\alpha,\beta}^{2,1} \cdot a^2b)\text{ in the case }\alpha+\beta,2\alpha+\beta\in\Phi. \tag{R4} \label{R4}
\end{align}
...
The aim of this section is to recall the formalism of Steinberg pro-groups, which has been used in~\cite{LSV20} and \cite{V20} to establish the centrality of $\K_2$ in a large number of cases. We will use this formalism in~\cref{sec:patching} as an ingredient in the proof of our patching theorem for Steinberg groups. 
We refer the reader to \cite[Section~6.1]{SK06} for a more thorough introduction to the formalism of ind-objects and pro-objects. We also refer to~\cite[\S~2]{LSV20} which contains a more detailed exposition of the material presented in this section.
...
\textstyle
T_\alpha(c/{h^s}) [u, v] = \bigl[x_\alpha(b/{h^s})\cdot u,\ c_{\overline{\iota}(u^{-1})}(x_\alpha(ah^{k - s})) \cdot v\bigr].
\end{equation}
...
T_\alpha(c'/{h^s})\, \bigl(T_\alpha(c/h^s)\, [u, v]\bigr) = T_\alpha\bigl((c+c')/h^s\bigr)\, [u, v].$
...
\begin{multline*}
 T_\alpha(c'/{h^s}) \bigl(T_\alpha(c/h^s) [u,\ v]\bigr) = T_\alpha(c'/h^s)[x_\alpha(b/h^s)\cdot u,\ c_{\overline{\iota}(u^{-1})}(x_\alpha(ah^{k-s})) \cdot v] = \\
 = [x_\alpha((b + b')/h^s)\cdot u,\ c_{\overline{\iota}(u^{-1}) \cdot x_\alpha(-b/h^s)}(x_\alpha(a'h^{m-s})) \cdot c_{\overline{\iota}(u^{-1})}(x_\alpha(ah^{k-s})) \cdot v] = \\ = [x_\alpha((b + b')/h^s)\cdot u,\  c_{\overline{\iota}(u^{-1})}(x_\alpha(a'h^{m-s} + ah^{k-s})) \cdot v] = T_\alpha\bigl((c+c')/h^s\bigr)[u, v]. \qedhere
\end{multline*}
\end{proof}

\begin{lemma} The operators $T_\alpha$ satisfy relations~\eqref{R3}. \end{lemma}
\begin{proof}
 Fix roots $\alpha, \beta \in \Phi$ such that $\alpha + \beta \in \Phi$ but $2\alpha + \beta, \alpha + 2\beta \notin \Phi$. Choose a decomposition $c = ah^k + b$ as in~\eqref{eq:decomp} and pick $n \geq k$ such that \[c_{ \overline{\iota}(u^{-1}) \cdot x_\beta(-b/h^s)}(x_\alpha(ah^{n-s})) = c_{\overline{\iota}(u^{-1})}\left(x_\alpha(ah^{n-s}) \cdot x_{\alpha+\beta}(N_{\alpha, \beta}\cdot ab \cdot h^{n-2s})\right).\]
\begin{multline}
 T_\alpha(c'/h^s) T_\beta(c/h^s) [u, v] = T_\alpha(c'/h^s) [x_\beta(b/h^s) \cdot u,\ c_{\overline{\iota}(u)}(x_\beta(ah^{k-s})) \cdot v] \\ LOL \end{multline}




Set $$\textstyle f = x_\alpha(c/h^s)\, x_\beta(c'/h^s)\text{ and }g = x_{\alpha + \beta}(N_{\alpha \beta}\cdot cc'/h^{2s})\, x_\beta(c'/h^s)\, x_\alpha(c/{h^s}).$$
Our goal is to show that $f[u, v] = g[u, v]$. Choose decompositions $c = ah^n + b$, $c' = a'h^n + b'$ and $cc' = (aa' h^n + ab' + a'b) h^n + bb'$ for sufficiently large $n$.
Then
$$\textstyle
