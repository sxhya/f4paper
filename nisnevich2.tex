...
\[ \pi_1^{\mathbb{A}^1}(G)(R) := \mathrm{Hom}_{\mathscr{H}_{*}(k)}(S^1 \wedge \mathrm{Spec}(R)_+, G) = \mathrm{KV}_2^{G}(R) = \K_2^G(R).\]
\end{corollary}
...
The representability of the stable $\K$-functors in $\mathscr{H}(k)$ well-known, see e.\,g. \cite[Corollary~3.4]{Ho05}, \cite[\S~4.3]{MV99}.
...
%Notice that the groups of values of the sheaf $\bm{\pi}_1^{\mathbb{A}^1}(G)$ and the presheaf $\pi_1^{\mathbb{A}^1}(G)$ agree on spectra of Henselian local rings of $k$-smooth varieties.
...
The assertion of~\cref{theorem:LP-for-K2} is likely to remain true for all $\Phi$ of rank $\geq 3$ without any assumptions on the characteristic of $k$. It is false for $\Phi$ of rank $2$, however, see~\cite{We12}. The bottleneck in our proof which leads to the restrictive assumptions $\ell\geq 7$ and $\mathrm{char}(k)\neq 2$ in the case $\Phi=\rD_\ell$ is the orthogonal Horrocks theorem for $\K_2$ proved in \cite{LS20}, see Theorem~1. In our future papers we plan to address this issue and obtain an Horrocks theorem which applies to all simply-laced $\Phi$ containing $\rA_4$ without the invertibility of $2$ assumption.

Recall from the~\cite[Theorem~5.3]{St71} that under the assumptions on the rank $\Phi$ stated in~\cref{theorem:LP-for-K2} the first two homology groups of $\St(\Phi, R)$ vanish. On the other hand, the main result of~\cite{LSV20} asserts that $\K_2^{G, Q}(R)$ is a central subgroup of $\St(\Phi, R)$ and, in particular, an abelian group.
From this and \S~IV.1 of~\cite{Kbook} it is easy to conclude that the group $\K_2^G(R)$ coincides with the unstable $\K_2$-group defined by means of Quillen's $+$-construction:
...
For a commutative ring $R$ and $a \in R$ consider the following commutative square:
  ...
  The ring $R \ltimes tR_a[t]$ can be defined either via the semidirect product construction (see e.\,g. \cite[Definition~3.2]{S15}) or as the pullback of $R \xrightarrow{\lambda_a} R_a \xleftarrow{ev(0)} R_a[t]$.
...
\begin{theorem} \label{lpb}
 ...
 \begin{enumerate}[label=\textnormal{(A\arabic*)}]
  \item \label{CFC} {\it $K$ is finitary}, i.\,e. commutes with filtered colimits. In other words if $A_i$ are Noetherian $k$-algebras and their colimit $A$ is also Noetherian, then $K(A) = \colim_i(K(A_i))$.
...
The choice of the name for~\eqref{LPP} is inspired by axiom (P3) from~\cite[Proposition~3.3.4]{AHW18}. It is easy to see that the application of $\mathrm{Spec}$ to the commutative square from~\ref{LPP} gives a distinguished Nisnevich square in $\catname{Sm}_{\mathrm{Spec}(B)}$ in the sense of~\cite[Definition~3.1.3]{MV99}. Notice also that our axioms can be slightly relaxed, so that the assertion of the Theorem becomes slightly stronger, see Remark~\ref{rem:relax} below.
\end{rem}
...
\[ \pi_1^{\mathbb{A}^1}(G)(R) := \mathrm{Hom}_{ \mathscr{H}_{*}(k)}(S^1 \wedge \mathrm{Spec}(R)_+, G) = \mathrm{KV}_2^{G}(R) = \K_2^G(R),\]
where $\mathscr{H}_*(k)$ denotes the pointed unstable $\mathbb{A}^1$-homotopy category over $k$.
\end{corollary}
\begin{proof}
...
Then the group $\St(\Phi, B)$ surjects onto the pullback of homomorphisms
$\lambda_{h}\colon \St(\Phi, A) \to \St(\Phi, A_{h})$, $\overline{\iota}\colon\St(\Phi, B_h) \to \St(\Phi, A_{h})$:
 \[ \begin{tikzcd}
...
\end{tikzcd} \]
 %and the sequence of pointed sets 
...
The asserion of Theorem~\ref{glueing} is equivalent to the exactness of the sequence of pointed sets 
...
\end{definition}
We will prove that $V$ injects into $\St(\Phi, A_{h})$. This clearly implies the exactness of~(\ref{stavrova-sequence}).

As a general rule, it is almost impossible to prove some injectivity statemnet using generators and relations directly. The usual trick here is to make $\St(\Phi, A_{h})$ act on some set in such a way that the images of different elements of $V$ act as different automorphisms. In fact, we will define the action of $\St(\Phi, A_{h})$ on $V$ itself.

\begin{proposition}
\label{action}
There exists a well-defined action $\cdot$ of $\St(\Phi, A_{h})$ on $V$ such that the natural map from $V$ to $\St(\Phi, A_{h})$ induced by~(\ref{stavrova-sequence}) is $\St(\Phi, A_{h})$-equivariant, where $\St(\Phi, A_{h})$ acts on itself by left translations. Moreover, for any $u\in\St(\Phi, B_h)$ and $v\in\St(\Phi, A)$ one has 
\begin{align}
\label{additional}
\lambda_{h}(v)\cdot[1,\,1]=[1,\,v]\quad\text{ and }\quad\overline\iota(u)\cdot[1,\,v]=[u,\,v].
\end{align}
\end{proposition}
This easily implies the following corollary and therefore Theorem~\ref{glueing}.
\begin{corollary}
\label{corollary-in-the-end}
The natural map from $V$ to $\St(\Phi, A_{h})$ is bijective, i.e., $V$ is a principal homogeneous set for $\St(\Phi, A_{h})$.
\end{corollary}
\begin{proof}
Since $[1,\,1]\in V$ is mapped to $1\in\St(\Phi,\,A_{h})$, we conclude that for any $g\in\St(\Phi,\,A_{h})$ an element $g\cdot[1,\,1]\in V$ is mapped to $g$, i.e., the map is surjective. On the other hand, in $[u,\,v]$ and $[u',\,v']$ are mapped to the same element of $\St(\Phi,\,A_{h})$, i.e., $\overline\iota(u)\lambda_{h}(v)=\overline\iota(u')\lambda_{h}(v')$, then using~(\ref{additional}) we have
$$
...
We will outline the construction of the action from Proposition~\ref{action} in the rest of this subsection. It consists of three steps.

{\bf Step~1.} For $g\in\St(\Phi,\,A_h)$, and $k$ large enough we will define a homomorphism $c_{g}\colon\St(\Phi,\,h^kA)\rightarrow\St(\Phi,\,A)$ imitating conjugation by $g$.

Recall that the definition of the Steinberg group $\St(\Phi,\,R)$ from Section~3 makes perfect sense for a ring $R$ without a unit, e.g., for $R=h^kA$ as above. In fact, a map $c_g$ can be defined in many different ways. Fortunately, for any two choices $c_g$ and $c_g'$ of such a map one can find $k'$ larger then $k$ such that the compositions 
 \[ \begin{tikzcd}
\St(\Phi,\,h^{k'}A)\ar{r}&\St(\Phi,\,h^kA)\ar[shift left=0.75ex, "c_g'"]{r}\ar[shift right=0.75ex, "c_g"']{r}&\St(\Phi,\,A)
\end{tikzcd} \]
become equal. %At this point it is convenient to recall the language of pro-completions before giving a more precise statemnet.
However, one cannot hope that there exists $k'$ such that {\it all} possible choices of maps $c_g$ become equal.

The construction of maps $c_g\colon\St(\Phi,\,h^kA)\rightarrow\St(\Phi,\,A)$ is in fact given in~\cite{LSV20}, however, it is not stated explicitely in that paper. It will be convenient to recall the language of pro-completions to give accurate references. This will be done in Subsections~\ref{pro-completion}-\ref{pro-groups}. 

\begin{rem}
The idea behind the construction of $c_g$ is quite simple. It is enough to define the maps $\St(\Phi,\,h^{k+t}A)\rightarrow\St(\Phi,\,h^tA)$ imitating the conjugation by $g=x_\beta(a/h^s)$ for $k$ sufficientlu large with respect to $s$ and any $t$, $a\in A$. To do so we put 
$$
...
\end{cases}
$$
according to Steinberg relations (R2) and (R3). The only issue here is to define the action of $x_\beta(a/h^s)$ on $x_{-\beta}(b/h^{k+t})$ which can also be done using the decomposition of $x_{-\beta}(b/h^{k+t})$ as a commutator with the use of relation (R3). We refer the reader to the introduction of~\cite{LSV20} for a more detailed explanation of this construction.
%$``(u^{-1}\cdot x_\alpha(ah^{k-s})\cdot u)"$
\end{rem}

{\bf Step~2.} For $\alpha\in\Phi$, $c/h^s\in A_h$ we will construct a map $T_\alpha(c/h^s)\colon V\rightarrow V$ imitating the action by left translation. 

To construct such a map we will use the condition $B/h\cong A/h$ of Theorem~\ref{glueing}, which is equivalent to the condition that for every $k\geq0$ one has $A = Ah^k + B$ and $Ah^k\cap B = Bh^k$. Therefore, we can first decompose $c\in A$ as $c=ah^k+b$ for $k$ large enough, $a\in A$, $b\in B$ and then define
$$
T_\alpha(ah^{k-s}+b/h^s)\cdot[u,\,v]=[x_\alpha(b/h^s)u,\,c_{u^{-1}}\big(x_\alpha(ah^{k-s})\big)\cdot v].
$$
Recall that $c_{u^{-1}}\big(x_\alpha(ah^{k-s})\big)$ of Step~1 gives a formal meaning to the expression $``(u^{-1}\cdot x_\alpha(ah^{k-s})\cdot u)"$. Obviously, we have to check that the above expression does not depend on the choices we made. This will be done in Subsection~\ref{sec:proof-glueing}.

{\bf Step~3.} We will prove that the maps $T_\alpha(c/h^s)\colon V\rightarrow V$ of Step~2 satisfy the Steinberg relations (R1)--(R3).

As a result we will get a well-defined action of $\St(\Phi,\,A_h)$ on $V$. We also should check that this action satisfies the additional conditions~(\ref{additional}) of Proposition~\ref{action}. This will be done in Subsection~\ref{sec:proof-glueing}.
...
We start by recalling the definition of the {\it pro-completion} $\catname{Pro}(\mathcal{C})$ of a category $\mathcal{C}$ (cf.~\cite[\S~2.1]{LSV20}). The pro-copmlition is taken with respect to a fixed small nonempty filtered category $\mathcal{I}$, and in our exaples $\mathcal I$ is always a diagram, i.e., for any two objects $i$, $j\in\mathcal I$ one has $$\mathrm{Hom}_{\mathcal I}(i,\,j)=\begin{cases}\emptyset,&\\{*}.&\end{cases}$$ In fact, we will only use $\mathcal I$ as in Example~\ref{main-example}.

By definition, the objects of $\catname{Pro}(\mathcal{C})$ are functors $X^{(\infty)}\colon\mathcal{I}^{\mathrm{op}} \to \mathcal{C}$. For $X^{(\infty)} \in 
...
In particular, for any pre-morphism $\eta$ and any function $\xi\colon \mathrm{Ob}(\mathcal{J})\to\mathrm{Ob}(\mathcal{I})$ such that $\mathrm{Hom}\big(\xi(j),\,\eta^*(j)\big)=*$ for all $j\in\mathrm{Ob}(\mathcal J)$, one can define a pre-morphism $\epsilon\colon X^{(\infty)} \to Y^{(\infty)}$ taking $\epsilon^*=\xi$ and $\epsilon_j$ to be the compositions $X^{(\xi(j))}\to X^{(\eta^*(j))}\xrightarrow{\eta_{j}}Y^{(j)}$. The pre-morphism $\epsilon$ is clearly equivalent to $\eta$. This construction of equivalent pre-morphism with ``enlarged'' $\eta^*(j)$ is often used in the sequel.
...
c) We denote by $\mathbb N$ the set of nonnegative integers. For a non-nilpotent element $h\in R$ denote by $h^{\mathbb{N}}$ the multiplicative subset $\{ h^n \mid n \in \mathbb{N} \} \subseteq R$. The pro-rng $R^{(\infty)}$ associated to $S=h^\mathbb{N}$ can be depicted of as the following diagram of rngs (by definition, $R^{(k)}$ is the principal ideal $h^kR$ and all the structure morphisms of $R^{(\infty)}$ are obvious inclusions):
...
\label{pro-groups}
We refer the reader to~\cite[\S~2]{LSV20} for a more detailed exposition of the material presented in this subsection. We call the pro-completion of the category $\mathcal C=\catname{Grp}$ from the above subsetion {\it the category of pro-groups} $\catname{Pro}(\catname{Grp})$.
...
%
%It is enough to verify the latter equalities for $\beta \in \Phi\setminus\{\alpha\}$ where 
\end{proposition}
The first assertion of the above proposition is proven in~\cite[Lemma~2.16]{LSV20}, and the second assertion follows from~\cite[Lemma~3.2]{LSV20}.
%
%
%by means of a universal property (cf.~\cite[Lemma~2.16]{LSV20}). For instance, $\St^{(\infty)}(\Phi, R)$ is ``generated'' by $x_\alpha$ in the sense that 
%
%(in fact, an even stronger result holds, see~\cite[Lemma~3.2]{LSV20}).
\begin{rem}
Moreover, given a collection of pro-group morphisms $f_\alpha \colon R^{(\infty)} \to G^{(\infty)}$ satisfying the ``pro-analogues'' of Chevalley commutator identities (see~\cite[Remark~2.15]{LSV20}), there exists a unique morphism $f \colon \St^{(\infty)}(\Phi, R) \to G^{(\infty)}$ such that $f_\alpha = f \circ x_\alpha$ for all $\alpha\in\Phi$. This fact is esential for the construction of homomorphism $\mathrm{conj}\colon \St(\Phi, R_S) \to \mathrm{Aut}(\St^{(\infty)}(\Phi, R))$ from Proposition~\ref{prop:conj-action} below performed in~\cite[Lemma~4.2, Proposition~4.3]{LSV20}, however, we will not use it directly in this paper.
\end{rem}
Moreover, for $u=r/s\in R_S$ $r\in R$, $s\in S$, $\beta\in\Phi$ one can define a pro-group morphism $x_\beta(u\cdot-)\colon R^{(\infty)}\rightarrow\St^{(\infty)}(\Phi, R)$ as an equivalense class of pre-morphism $\eta$ with $\eta^*(t)=(st)$ and $\eta_t(r)=x_\alpha(ur)$ (see~\cite[Subsection~4.2]{LSV20} for more details). The following proposition is a restatemnt of~\cite[Proposition~4.3]{LSV20}.
\begin{proposition}\label{prop:conj-action}
 ...
 \begin{align}
 \label{conj1}
&\mathrm{conj}(x_\alpha)(x_\beta)=x_\beta,\quad\text{ for }\alpha+\beta\not\in\Phi\cup0,\\
 \label{conj2}
&\mathrm{conj}(x_\alpha)(x_\beta)=x_{\alpha+\beta}(N_{\alpha\beta}u\cdot-)\cdot x_\beta(-),\quad\text{ for }\alpha+\beta\in\Phi.
 \end{align}
\end{proposition}
\begin{proof}
...
Since $\mathrm{conj}$ is a homomorphism, it is enough to consider the case $g = x_\alpha(u)$, $\alpha \in \Phi$, $u \in R_S$. In view of Proposition~\ref{generators}, to verify the commutativity of~\eqref{eq:conj-pis} it is enough to verify the equalities $\pi_S \circ \mathrm{conj}(g) \circ x_\beta= {}^g\!(-) \circ \pi_S \circ x_\beta$ for $\beta\neq -\alpha$.
 It remains to see that the fulfillment of these equalities is a direct consequence of~(\ref{conj1})--(\ref{conj2}). %the definition of $\mathrm{conj}(g) \circ x_\beta$ (the formulas for these morphisms are given by formulas (4.5)--(4.6) of~\cite{LSV20}). 
 \end{proof}
...
% Notice also that the root subgroup morphisms $x_\alpha \colon R^{(\infty)} \to \St^{(\infty)}(\Phi, R)$ satisfy the identity $\mathrm{conj}(x_\alpha(u)) \circ x_\alpha = x_\alpha$ for any $u \in R_S$.
%\end{rem}
...
\end{proposition}
\begin{proof}
As before, we may suppose that \(g = x_\alpha(a/s)\) for some \(\alpha \in \Phi\), \(a \in R\), \(s \in S\). By Propostion~\ref{generators}\,b) %\cite[Lemma~3.2]{LSV20} 
it suffices to check that the long paths in the diagram coincide after precomposition with \(x_\beta \colon R^{(\infty)} \to \St^{(\infty)}(\Phi, R)\) for all \(\beta \neq -\alpha\). This follows from~(\ref{conj1})--(\ref{conj2}) %the definition before~\cite[Proposition~4.2]{LSV20}
...
Now let us specialize the above results to a particular situation that we will encounter in~\cref{sec:proof-glueing}.%, see Example~\ref{main-example}\,c).
...
Now we will acomplish the plan outlined in Subsection~\ref{plan}. Recall that we let $\St(\Phi, B)$ act on $\St(\Phi, B_h) \times \St(\Phi, A)$ on the left via the formula 
...
In Subsection~\ref{plan} we divided the proof of this fact in three steps, which we will acomplish below.
...
{\bf Step~1.} The first step was the definition of homomorphisms $c_{g}\colon\St(\Phi,\,h^kA)\rightarrow\St(\Phi,\,A)$ imitating conjugation by $g\in\St(\Phi,\,A_h)$ for $k$ large enough. It was aready  performed in Subsections~\ref{pro-completion}--\ref{pro-groups}. 

For the further construction, we  
%
%The first step in this direction is to construct an action of $\St(\Phi, A_{h})$ on $V$. 
%
fix some $u \in \St(\Phi, B_h)$, $v \in \St(\Phi, A)$, $\alpha \in \Phi$, and $c/{h^s} \in A_{h}$. 
...
Since $a'h^l = ah^k + dh^l$ and $h$ is not a zero divisor (because $A$ is a domain) we obtain that $a'h^{l-s} = dh^{l-s} + ah^{k-s}$ therefore
\begin{align*}
...
{\bf Step~3.} The third step of the proof is to verify that the above operators $T_\alpha(c/h^s)$ specify an action of $\St(\Phi, A_{h})$ on $V$ satisfying~(\ref{additional}).
...
%
%this follows from the definition of the composition of pre-morphisms and Remark~\ref{rem:conj-root-action}. 
...
$$
and therefore the first components of the expressions coincides, and using
$$
...
\begin{lemma} The constructed action satisfies the conditions~\ref{additional}. 
\end{lemma}
\begin{proof}
Indeed, using the fact that the operators $T_\alpha(a)$ are well-defined by Lemma~\ref{well-def}, for $a\in A$ we can choose a decomposition $a=ah^0+0$ and for $u=1\in\mathrm{St}(\Phi,\,B_h)$ we can choose $c_{1}$ to be the ifentity map. Then by the very definition
$$
