...
\begin{enumerate}
 \item $f \circ c_g = c_{\overline{f}(g)} \circ f$,
 \item $\lambda_h^0 (c_g (x)) = g \cdot \lambda_h^n(x) \cdot g^{-1}$ for every $x \in \St(\Phi, h^nB)$.
 \item $\lambda_h^0 (c_{\overline{f}(g)}(y)) = g \cdot \lambda_h^n(y) \cdot \overline{f}(g)^{-1}$ for every $y \in \St(\Phi, f(h)^nA)$.
\end{enumerate}
...
First of all, recall that the condition $A/hA = B/hB$ is equivalent to the fact that for every $n \geq 0$ one has $A = Ah^n + B$ and $Ah^n \cap B = Bh^n$.
