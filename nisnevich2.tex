...
Recall from the~\cite[Theorem~5.3]{St71} that under the assumptions on the rank $\Phi$ stated in~\cref{theorem:LP-for-K2} the first two homology groups of $\St(\Phi, R)$ vanish. On the other hand, the main result of~\cite{LSV20} asserts that $\K_2^G(R)$ is a central subgroup of $\St(\Phi, R)$ and, in particular, an abelian group.
...
The choice of the name for~\eqref{LPP} is inspired by axiom (P3) from~\cite[Proposition~3.3.4]{AHW18}. It is easy to see that the application of $\mathrm{Spec}$ to the commutative square from~\ref{LPP} produces a distinguished Nisnevich square in $\catname{Sm}/\mathrm{Spec}(B)$ in the sense of~\cite[Definition~3.1.3]{MV99} (for Noetherian $B$ of finite Krull dimension). Notice also that our axioms can be slightly relaxed, so that the assertion of the Theorem becomes slightly stronger, see Remark~\ref{rem:relax} below.
\end{rem}
...
The geometric content of this theorem can be heuristically explained as follows. Let $X=\mathrm{Spec}(R)$ be $k$-smooth affine scheme and $X_f=\mathrm{Spec}(R_f)$ its principal open set, $A=\mathcal O_{X,M_1,\ldots, M_n}$. The left-hand square of the diagram~(\ref{eq:panin-diag}) defines an isomorphism of motivic spaces (i.e. Nisnevich sheaves) $\mathrm{Spes}(S)/\mathrm{Spes}(S_{\tau(h)})\cong(\mathbb A^1\times\mathrm{Spec}(A))/(\mathbb A^1\times\mathrm{Spec}(A))_h$. Then the theorem asserts that the canonical map $\mathrm{Spec}(\mathcal O_{X,M_1,\ldots, M_n})\to X/X_f$ factors through $(\mathbb A^1\times\mathrm{Spec}(\mathcal O_{X,M_1,\ldots, M_n}))/(\mathbb A^1\times\mathrm{Spec}(\mathcal O_{X,M_1,\ldots, M_n}))_h$.
...
Notice that every regular ring containing a field $k$ is geometrically regular over the prime subfield of $k$ which is perfect. Now the assertion of~\cref{theorem:LP-for-K2} in the simply-laced case ($\Lambda = P(\Phi)$) follows from~\cref{lpb}.
...
\begin{tikzcd} 
  &  1\ar[r]\ar[d]  & \mathrm{St}(\Phi, R)   \ar{d} \ar[r]  & \mathrm{St}(\Phi, R) \ar{d} \ar[r]  & 1 \\
    ...
    \end{tikzcd}
$$
and similar one with $R[t]$ instead of $R$ we obtain the rows of the diagram
...
\begin{corollary}
\label{cor:+=A1}
Let $\Phi$, $\Lambda$ and $k$ be as in~\cref{theorem:LP-for-K2}. Then for any smooth $k$-algebra $R$ and $G = \mathrm{G}_{\Lambda}(\Phi, -)$ one has
$$
\K_{2}^{G,Q}(R) = \mathrm{KV}_2^G(R),
$$
where $\K_2^{G, Q}(R)$ denotes $\mathrm K_2$-group defined via Quillen's $+$-construction, see~(\ref{eq:H2-K2}). In other words, the definitions of the unstable $\K_2$-functors a la Quillen and a la Karoubi--Villamayor agree.
\end{corollary}
...
Consider the natural map $V \to \St(\Phi, A_h)$ induced by $\overline{\iota}$ and $\lambda_h$. If we show that this map is injective the exactness~\eqref{stavrova-sequence} would follow. The main ingredient in the proof is the following
...
The natural map from $V\to \St(\Phi, A_{h})$ is bijective, i.e., $V$ is a principal homogeneous set for $\St(\Phi, A_{h})$. 
\end{corollary}
\begin{proof}
Since $[1,\,1]\in V$ is mapped to $1\in\St(\Phi,\,A_{h})$, we conclude that for any $g\in\St(\Phi,\,A_{h})$ an element $g\cdot[1,\,1]\in V$ is mapped to $g$, i.e., the map is surjective. On the other hand, if $[u,\,v]$ and $[u',\,v']$ are mapped to the same element of $\St(\Phi,\,A_{h})$, i.e., $\overline\iota(u)\lambda_{h}(v)=\overline\iota(u')\lambda_{h}(v')$, then using~(\ref{additional}) we have
$$
...
{\bf Step~1.} For $g\in\St(\Phi,\,A_h)$ and sufficiently large $k$ we define a homomorphism $c_{g}\colon\St(\Phi,\,h^kA)\rightarrow\St(\Phi,\,A)$ which models conjugation by $g$ on the level of Steinberg groups (in particular, for all $x\in \St(\Phi,\, h^kA)$ one has the identity $\pi(c_g(x)) = \pi(g) \pi(x) \pi(g^{-1})$ in $\mathrm{G}_\Lambda(\Phi, A_h)$). 
The map $c_g$ is constructed in~\cite{LSV20} using the technique of pro-groups which is discussed in Subsections~\ref{pro-completion}-\ref{pro-groups}. 

%There is no canonical way to construct the homomorphism $c_g$, however, any $c_g$ and $c_g'$ one can find $k'$ larger then $k$ such that the composite arrows of the following diagram are equal: \[ \begin{tikzcd} \St(\Phi,\,h^{k'}A)\ar{r}&\St(\Phi,\,h^kA)\ar[shift left=0.75ex, "c_g'"]{r}\ar[shift right=0.75ex, "c_g"']{r}&\St(\Phi,\,A). \end{tikzcd} \] On the other hand, one cannot hope for a number $k'$ such that {\it all} possible choices of maps $c_g$ become equal.
...
\end{cases}
$$
This agrees with Steinberg relations \eqref{R2} and \eqref{R3}. The difficulty here is to define  $c_{x_\beta(a/h^s)}(x_{-\beta}(b/h^{k+t}))$, but this can be done by decomposing $x_{-\beta}(b/h^{k+t})$ into a commutator by means of~\eqref{R3}. We refer the reader to the introduction of~\cite{LSV20} for a more detailed explanation of this construction.
%$``(u^{-1}\cdot x_\alpha(ah^{k-s})\cdot u)"$
\end{rem}

{\bf Step~2.} For $\alpha\in\Phi$, $c/h^s\in A_h$ we construct a map $T_\alpha(c/h^s)\colon V\rightarrow V$ imitating the action by left translation. 
...
\end{equation}
Here we invoke the homomorphism $c_{u^{-1}}$ constructed on Step 1.
%Recall that the expression $c_{u^{-1}}\big(x_\alpha(ah^{k-s})\big)$ is just a formal way to say $``u^{-1}\cdot x_\alpha(ah^{k-s})\cdot u"$.
Obviously, we have to check that the the right-hand side of~\eqref{eq:step2} does not depend on the choices of $k$, $c_g$ etc. This is achieved in Lemma~\ref{well-def}.
...
As a result, we get a well-defined action of $\St(\Phi,\,A_h)$ on $V$. We also should check that this action satisfies the additional conditions~(\ref{additional}) of Proposition~\ref{action}. This is achieved in Subsection~\ref{sec:proof-glueing}.
...
We start by recalling the definition of the {\it pro-completion} $\catname{Pro}(\mathcal{C})$ of a category $\mathcal{C}$ (cf.~\cite[\S~2.1]{LSV20}). The pro-completion is taken with respect to a fixed small nonempty filtered category $\mathcal{I}$, and in our examples $\mathcal I$ is always a diagram, i.\,e., for any two objects $i$, $j\in\mathcal I$ one has $$\mathrm{Hom}_{\mathcal I}(i,\,j)=\begin{cases}\emptyset,&\\{*}.&\end{cases}$$ In fact, in all our examples $\mathcal I$ is as in Example~\ref{main-example} below.
...
\begin{proposition}\label{prop:conj-action}
 ...
 \begin{align}
 \label{conj1}
&\mathrm{conj}(x_\alpha) \circ x_\beta =x_\beta,\quad\text{ for }\alpha+\beta\not\in\Phi\cup0,\\
 \label{conj2}
&\mathrm{conj}(x_\alpha) \circ x_\beta =x_{\alpha+\beta}(N_{\alpha\beta}u\cdot-)\cdot x_\beta(-),\quad\text{ for }\alpha+\beta\in\Phi.
 \end{align}
\end{proposition}
\begin{proof}
...
Finally, we move on to Step~3 of the plan, namely we verify that the above operators $T_\alpha(c/h^s)$ specify an action of $\St(\Phi, A_{h})$ on $V$ satisfying~(\ref{additional}).
