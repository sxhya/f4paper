...
As a consequence~\cref{theorem:LP-for-K2} we also obtain the following
\begin{corollary} \label{cor:various-facts}
For $k$ with $\mathrm{char}(k)\neq 2$, $\ell \geq 7$ and $n\geq 0$ one has
\begin{align*} 
\mathrm H_2(\mathrm{Spin}_{2\ell}(k[t_1,\ldots,t_n]),\,\mathbb Z\big) =& \K^\mathrm{M}_2(k),\\
\mathrm H_2(\mathrm{SO}_{2\ell}(k[t_1,\ldots,t_n]),\,\mathbb Z\big) =& \K^\mathrm{M}_2(k)\times \ZZ/2\ZZ. \end{align*}
The group $\mathrm{Spin}_{2\ell}(k[t_1,\ldots, t_n])$ admits presentation as in~\cite[\S~6]{St67}, i.\,e. the presentation by means of generators $x_\alpha(\xi)$, $\xi \in k[t_1,\ldots, t_n]$ relations~\eqref{R1}--\eqref{R3} and relations $h_\alpha(u)h_\alpha(v) = h_\alpha(uv)$, where $u, v\in k^\times$, cf. also with e.\,g.~\cite[\S~5]{Ma69}.
\end{corollary}
...
We denote by $\E^G(\Phi, R)$ the {\it elementary subgroup} of $G = \mathrm{G}_\Lambda(\Phi, R)$. It is defined as the abstract subgroup generated by elementary root unipotents $t_\alpha(\xi)$, $\alpha \in \Phi$, $\xi \in R$, see e.\,g.~\cite{St71, St78, VZ20, Abe83,}. It is clear that $\E^G(R)$ is the image of the homomorphism $\pi$ from~\eqref{eq:main-exactSeq}. We denote by $\E_\mathrm{sc}(\Phi, R)$ the elementary subgroup of $\Gsc(\Phi, R)$.
...
It remains to verify that the assertion of~\cref{theorem:LP-for-K2} holds for the Chevalley group $G = G_\Lambda(\Phi, -)$ of nonsimply-connected type. Notice that such group fits into the sequence
\[ \begin{tikzcd} 1 \ar[r] & M(R) \ar[r] & \Gsc(\Phi, R) \ar[r] & \mathrm{G}(R), \end{tikzcd} \]
where $M$ is either $\mu_n$ for some $n$ or $\mu_2\times \mu_2$ (cf.~\cite[\S~3]{St67}).
It is clear that $M(R)$ is contained in the maximal torus of $\Gsc(\Phi, R)$ and therefore in $\E_\mathrm{sc}(\Phi, R)$, so we can replace $\Gsc(\Phi, R)$ with $\E_\mathrm{sc}(\Phi, R)$ in the above sequence. Invoking the snake lemma we obtain the diagram
...
\begin{proof}[Proof of~\cref{cor:various-facts}]
 Set $R = k[t_1,\ldots t_n]$. Recall from~\cite{Abe83} that $\K_1(\rD_\ell, R) = \K_1(\rD_\ell, k) = 1$, so $\mathrm{Spin}_{2\ell}(R) = \E_\mathrm{sc}(\rD_\ell, R)$. The presentation a la Steinberg for $\mathrm{Spin}_{2\ell}(R)$ now follows from~\cref{theorem:LP-for-K2} and Matsumoto's theorem~\cite[Theorem~5.10]{Ma69}. The computation of the second homology group for $\mathrm{Spin}_{2\ell}(R)$ now follows from~\eqref{eq:H2-K2}. To deduce the analogous formula for $G = \mathrm{SO}_{2\ell}(R)$ it remains to notice that the upper row of~\eqref{eq:snake-A1} splits in the category of abelian groups. It certainly splits in the category of groups, see~\cite[Theorem~7.2.12]{HOM}. The group $\K_2(\Phi, R)$ is abelian by~\cite{LS17} (or~\cite{LSV20}). Finally, the group $\K_2^G(R)$ is abelian by~\cite[\S~7(v)]{St67}. \end{proof}
...
\end{theorem}
\begin{proof}
First of all, notice that by the homotopy invariance for $\K_1$ (see e.\,g.~\cite[Theorem~1.1]{Sta20}) the simplicial group $\K_1^G(R[\Delta^\bullet])$ is discrete. Applying the exact sequence~\eqref{eq:simplicial-les} to the canonical morphism $G(R[\Delta^\bullet]) \twoheadrightarrow \K_1^G(R[\Delta^\bullet])$, we obtain that $\pi_1(G( R[\Delta^\bullet])) \cong \pi_1(\E^G(R[\Delta^\bullet]))$.

Now consider the simplicial map $\mathrm{st}_\bullet \colon \St(\Phi, R[\Delta^\bullet]) \to \E^G(R[\Delta^\bullet])$ given by canonical projections $\mathrm{st}$ in each degree. The application of~\eqref{eq:simplicial-les} yields an exact sequence of groups
\[
\pi_1\bigl(\St(\Phi, R[\Delta^\bullet])\bigr) \to \pi_1\bigl(\E^G(R[\Delta^\bullet])\bigr) \to \pi_0\bigl(\K_2^G( R[\Delta^\bullet])\bigr) \to \pi_0\bigl(\St(\Phi, R[\Delta^\bullet])\bigr).
\]
