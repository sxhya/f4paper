...
\[ \begin{tikzcd} \K_2(\Phi, R) \ar[r] & \St(\Phi, R) \ar[r] & \mathrm{G}_\mathrm{sc}(\Phi, R) \ar[r] & \K_1(\Phi, R), \end{tikzcd} \]
the rightmost and the leftmost terms of which are called in~\cite{St78} the {\it unstable $\K$-functors modeled on Chevalley groups}. The aim of the present paper is to show the following theorem, which can be considered as the solution to the $\K_2$-analogue of the geometric case of the Bass--Quillen conjecture.
...
\end{theorem} The $\mathbb{A}^1$-invariance for the unstable $\K_1$-functor modeled on $\Phi$ of rank $\geq 2$ has been established in~\cite{Abe83} and has been subsequently generalized to isotropic reductive groups, cf.~\cite[Theorem~1.1]{Sta20}. An exposition of the classical Lindel--Popescu theorem for projective modules can be found in~\cite{Sw98}. 
...
The claim of~\cref{theorem:LP-for-K2} is likely to remain valid for all $\Phi$ of rank $\geq 3$ without any assumptions on the characteristic of $k$. It has no chance of being true for $\Phi$ of rank $2$, see~\cite{We12}. The bottleneck in our proof which leads to the (too restrictive) assumption $\ell\geq 7$ in the orthogonal case is the orthogonal Horrocks theorem for $\K_2$, see~\cite{LS20}. In the future installments of the series of papers we plan to address this issue and obtain a stronger Horrocks theorem for all simply-laced $\Phi$ containing $\rA_4$.

The main ingredient in the proof of~\cref{theorem:LP-for-K2} is~\cref{lpb}, which reduces the proof of $\mathbb{A}^1$-invariance to checking a short list of axioms. A key ingredient in the proof of~\cref{lpb}, in turn, is a recent result of I.~Panin, see~\cite[Theorem~2.5]{Pa19}. On the other hand, one of the axioms to-be-checked is the so-called {\it weak affine Nisnevich excision property} for Steinberg groups. The verification of this property for simply-laced Steinberg groups is another new result of the paper, see~\cref{sec:patching}. Unlike M.~Tulenbaev, who used the technique of van der Kallen's ``another presentation'' in the proof of the weaker Zariski excision property, in our proof we use the new technique of pro-groups introduced by the third-named author in~\cite{Vor1} (see also~\cite{LSV20, V20}).

As a corollary of~\cref{theorem:LP-for-K2}, we show that the group $\K_2(\Phi, R)$ can be interpreted as the fundamental group of the Chevalley--Demazure group scheme in the unstable $\mathbb{A}^1$-homotopy category $\mathcal{H}^{\mathbb{A}^1}_{k}$ of Morel and Voevodsky~\cite{MV99}.
...
\end{corollary}
The above result is analogous to the computation of $\pi_0$ of an isotropic reductive group due to A.~Stavrova, see~\cite[Theorem~5.5]{Sta20}. The analogue of~\cref{cor:motivic-pi1} in the special case $R = k$ has also been obtained in~\cite{VW16} in the context of reductive groups of isotropic rank $\geq 2$.

Apart from~\cref{theorem:LP-for-K2} another key ingredient in the proof of~\cref{cor:motivic-pi1} is the computation of the fundamental group of the simplicial group $\St(\Phi, R[\Delta^\bullet])$, see \cref{prop:pi1-StDelta}. This computation can be considered as a more precise version of the calculation given in~\cite[Proposition~3.2]{VW16}.

The technique used in the proof of~\cref{lpb} allows one to obtain the following characterization of $\K_2(\Phi, R)$ as a byproduct.
...
Combining the above result with \cite[Theorem~5.10]{Ma69} one can obtain that under the assumptions of~\cref{theorem:LP-for-K2} $\K_2(\Phi, A)$ embeds into the Milnor $\K_2$-group $\K_2^\mathrm{M}(\mathrm{Frac}(A))$.

As a consequence of the triviality of $\K_1(\rD_\ell, k[t_1,\ldots, t_n])$ and the Corollary of the main result of~\cite{LSV20} one can also obtain the following
\begin{corollary}
