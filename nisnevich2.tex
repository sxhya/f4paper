...
Let $\iota \colon B \to A$ be an inclusion of integral domains and let $h \neq 0$ be an element of $B$.
We denote by \(\mathbb N\) the set of nonnegative integers.
Consider the multiplicative subset $h^{\mathbb{N}} = \{ h^n \mid n \in \mathbb{N} \} \subset B \subseteq A$ and denote by $\overline{\iota}$ the canonical homomorphism $B_h \to A_h$.
It is clear that $h^\mathbb{N} \subseteq B$ and $h^\mathbb{N} \subseteq A$ and $f$ satisfy the requirements mentioned before Proposition~\ref{prop:functoriality} (e.\,g. $\iota^*(h^n) = h^n$), so there exists a morphism $\iota^{(\infty)}$ of pro-rngs $B^{(\infty)} \to A^{(\infty)}$ and the corresponding morphism of Steinberg pro-groups $\iota^{(\infty)}\colon\St^{(\infty)}(\Phi, B) \to \St^{(\infty)}(\Phi, A)$ satisfying the conclusion of Proposition~\ref{prop:functoriality}.

\begin{corollary}
\label{vorcor}
For $\iota \colon B \hookrightarrow A$ and $h \in B$ as above and every $g \in \St(\Phi, B_h)$ there exists $n = n(g) \in \mathbb{N}$ 
...
\begin{align}
 ...
 \label{eq:strictB} \lambda_{h, B}^0 (c_g (x)) = g \cdot \lambda_{h, B}^n(x) \cdot g^{-1} &\text{ for every }x \in \St(\Phi, h^nB), \\
 \label{eq:strictA} \lambda_{h, A}^0 (c_{\overline{\iota}(g)}(y)) = \overline{\iota}(g) \cdot \lambda_{h, A}^n(y) \cdot \overline{\iota}(g)^{-1} &\text{ for every }y \in \St(\Phi, h^nA).
\end{align}
Here for $k \in \mathbb{N}$ we denote by $\lambda_{h, B}^k$ (resp. $\lambda^k_{h, A}$) the homomorphism of Steinberg grops induced by the obvious rng homomorphism $h^kB \to B_h$ (resp. $h^kA \to A_{h}$).
\end{corollary}
\begin{proof}
