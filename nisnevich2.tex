...
\received{1 Month 202X}
\revised{1 Month 202X}
\accepted{1 Month 202X}
...
We call the images of the arrows of $\mathcal{I}$ under $X^{(\infty)}$ the {\it structure morphisms} of $X^{(\infty)}$.
...
\begin{rem}
  Suppose that the category $\mathcal{I}$ is a {\it diagram}, i.\,e., for any two objects $i$, $j\in\mathcal I$ the hom-set $\mathrm{Hom}_{\mathcal I}(i,\,j)$ is either empty or one-element. The index category of every pro-object that we encounter in the sequel is a diagram (in fact, it is the same diagram as in~Example~\ref{main-example}) below.

  Let $\eta \colon X^{(\infty)} \to Y^{(\infty)}$ be a pre-morpism and $\xi\colon \mathrm{Ob}(\mathcal{J})\to\mathrm{Ob}(\mathcal{I})$ an arbitrary function such that $\mathrm{Hom}\big(\xi(j),\,\eta^*(j)\big)=*$ for all $j\in\mathrm{Ob}(\mathcal J)$, one can define a ''restricted`` pre-morphism $\epsilon\colon X^{(\infty)} \to Y^{(\infty)}$ by taking $\epsilon^*=\xi$ and setting $\epsilon_j := (X^{(\xi(j))}\to X^{(\eta^*(j))}\xrightarrow{\eta_{j}}Y^{(j)})$. The pre-morphism $\epsilon$ is clearly equivalent to $\eta$. This construction of an equivalent restricted pre-morphism will often be used in the sequel.
\end{rem}
