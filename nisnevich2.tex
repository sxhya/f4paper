...
In view of~\cite[Lemma~3.2]{LSV20}, to verify the commutativity of~\eqref{eq:conj-pis} it is enough to verify the equalities $\pi_S conj(g) x_\beta= (-)^g\pi_S x_\beta$ for $\beta\neq \pm\alpha$, where $x_\beta\colon R^{(\infty)} \to \St^{(\infty)}(\Phi, R)$ are ``root subgroup morphisms'' defined in~\cite[\S~2.4]{LSV20}.
...
\begin{rem}
Unwinding the definitions, we see that the commutativity of~\eqref{eq:conj-pis} is equivalent to the fact that for every $g\in \St(\Phi, R_S)$ there exists $s_0 \in S$ and a homomorphism $c_g\colon \St(\Phi, s_0 R) \to \St(\Phi, R)$ such that $\lambda_S(c_g(x)) = g\lambda_S(i(x))g^{-1}$ for all $x\in \St(\Phi, s_0 R)$, where $i$ denotes the homomorphism $\St(\Phi, s_0 R) \to \St(\Phi, R)$ induced by the obvious embedding $s_0R \hookrightarrow R$.
\end{rem}
...
It is clear that $f$ induces a ring homomorphism $R_S \to R'_{S'}$, which we denote by $\overline{f}$.
Now suppose that there is a map $f^* \colon S' \to S$ such that $t$ divides $f(f^*(t))$ for all $t \in S'$ (this means, in particular, that $R'_{f(S)} \cong R'_{S'}$). We claim that $f^*$ and the obvious embeddings $\{f(f^*(t)R) \hookrightarrow tR'\}_{t\in S'}$ specify a pre-morphism of pro-rngs. We denote by $f^{(\infty)}$ the corresponding pro-rng morphism $R^{(\infty)} \to R'^{(\infty)}$. We reuse the notation $\overline{f}$, $f^{(\infty)}$ for the induced morphisms of Steinberg groups and pro-groups.
...
\begin{prop}
 ...
 \[ \begin{tikzcd} \St^{(\infty)}(\Phi, R) \ar{r}{conj(g)} \ar{d}{f^{(\infty)}} & \St^{(\infty)}(\Phi, R) \ar{d}{f^{(\infty)}} \\ \St^{(\infty)}(\Phi, R') \ar{r}{conj(\overline{f}(g))} & \St^{(\infty)}(\Phi, R'). \end{tikzcd} \]
\end{prop}
\begin{proof}
