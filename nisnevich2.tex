...
\address{Euler Mathematical Institute, 14th Line V.O., 29B, Saint-Petersburg 199178 Russia}
...
{\bf TODO: } Refer to some sources of info.
...
\end{equation}
The application of a functor $G\colon\catname{Ring}\rightarrow\catname{Grp}$ to $R[\Delta^\bullet]$ yields a simplicial group $G(R[\Delta^\bullet])$, which in the context of $\mathbb{A}^1$-homotopy theory is usually called the {\it simplicial resolution} of $G$ and is denoted by $\mathrm{Sing}^{\mathbb{A}^1}_\bullet(G)(R)$. Recall also, that the {\it unstable Karoubi--Villamayor K-functor} $\mathrm{KV}_{n+1}(\Phi, R)$ attached to the Chevalley--Demazure group scheme $\mathrm{G}_\mathrm{sc}(\Phi, -)$ is, by definition, the $n$-th simplicial homotopy group of $G(R[\Delta^\bullet])$ (cf.~\cite[\S~4.3]{AHW18}).

The aim of this subsection is to show that Stein's group $\K_2(\Phi, R)$ can be interpreted as the fundamental group of the simplicial group $\Gsc(\Phi, R[\Delta^\bullet])$ under the assumption that $\Phi$ and $R$ satisfy the requirements of~\cref{theorem:LP-for-K2}. This is achieved in~\cref{theorem:pi1-GRDelta} below. This result parallels the computation of the fundamental group of the simplicial group $G(k[\Delta^\bullet])$ modeled on an arbitrary isotropic reductive group $G$, see~\cite[Proposition~3.2]{VW16}. As a corollary of~\cref{theorem:pi1-GRDelta} and a general representability result of A.~Asok, M.~Hoyois and M.~Wendt we obtain an interpretation of $\K_2(\Phi, R)$ in terms of $\mathbb{A}^1$-homotopy theory, see~\cref{cor:motivic-pi1}.
...
The main result of this section is the following patching theorem for Steinberg groups.
...
%The main ingredient in the proof of~\cref{glueing} is the formalism of Steinberg pro-groups, which has been orinally invented by the third-named author in~\cite{V20}.
We refer the reader to \cite[Section~6.1]{SK06} for a introduction to the formalism of ind-objects and pro-objects. We also refer to~\cite[\S~2]{LSV20} which contains a more detailed exposition of the material presented in this subsection.
