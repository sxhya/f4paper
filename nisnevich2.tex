...
Choose a decomposition $c = ah^k + b$ as in~\eqref{eq:decomp} for some sufficiently large $k \geq s$.
We claim that there exists a sufficiently large number $n \geq k$ such that $c_{x_\alpha(b/h^s) \cdot \overline{\iota}(u)}(x_\alpha(a'h^{n-s})) = c_{\overline{\iota}(u)}(x_\alpha(a'h^{n-s}))$ for all $a'\in A$. Indeed, this follows from the definition of the composition of pre-morphism and Remark~\ref{rem:conj-root-action}. 
