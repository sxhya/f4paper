...
\textstyle
T_\alpha(c/{h^s}) [u, v] = \bigl[x_\alpha(b/{h^s})\cdot u,\ c_{\overline{\iota}(u)}(x_\alpha(ah^{k - s})) \cdot v\bigr].
\end{equation}
...
\begin{equation} \label{eq:wd1} \textstyle x_\alpha(b/{h^s})\, u = x_\alpha({b'}/{h^s})\, u \cdot u^{-1}\, x_\alpha(dh^{l-s})\, u =  x_\alpha({b'}/{h^s})\, u \cdot \lambda_h \bigl(c_u(x_\alpha(dh^{l-s}))\bigr). \end{equation}
...
We claim that there exists a sufficiently large number $m \geq k$ such that $c_{x_\alpha(b/h^s) \cdot \overline{\iota}(u)}(x_\alpha(a'h^{m-s})) = c_{\overline{\iota}(u)}(x_\alpha(a'h^{m-s}))$ for all $a'\in A$. Indeed, this follows from the definition of the composition of pre-morphisms and Remark~\ref{rem:conj-root-action}. 
Now choose a decomposition $c' = a' h^n + b'$.
The assertion now follows from the following calculation:
\begin{multline*}
 T_\alpha(c'/{h^s}) \bigl(T_\alpha(c/h^s) [u, v]\bigr) = T_\alpha(c'/h^s)[x_\alpha(b/h^s)\cdot u, c_{\overline{\iota}(u)}(x_\alpha(ah^{k-s})) \cdot v] = \\
 = [x_\alpha((b + b')/h^s)\cdot u, c_{x_\alpha(b/h^s)\overline{\iota}(u)}(x_\alpha(a'h^{n-s})) c_{\overline{\iota}(u)}(x_\alpha(ah^{k-s})) \cdot v] = \\ = [x_\alpha((b + b')/h^s)\cdot u,\  c_{\overline{\iota}(u)}(x_\alpha(a'h^{n-s} + ah^{k-s})) \cdot v] = T_\alpha\bigl((c+c')/h^s\bigr)[u, v]. \qedhere
\end{multline*}
\end{proof}

\begin{lemma} The operators $T_\alpha$ satisfy relations~\eqref{R3}. \end{lemma}
\begin{proof}
Fix $\alpha, \beta \in \Phi$ such that $\alpha + \beta \in \Phi$ and $2\alpha + \beta, \alpha + 2\beta \notin \Phi$. Set $$\textstyle f = x_\alpha(c/h^s)\, x_\beta(c'/h^s)\text{ and }g = x_{\alpha + \beta}(N_{\alpha \beta}\cdot cc'/h^{2s})\, x_\beta(c'/h^s)\, x_\alpha(c/{h^s}).$$
