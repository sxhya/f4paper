...
We define an action of the generator $x_\alpha(c/h^s)$ of $\St(\Phi, A_h)$ on $V$ via the formula:
...
\end{lemma}
\begin{proof}
First, let us demonstrate the independence of the choice of a decomposition for $c$. 
Indeed, set $l := n + s$ and $c = a' h^{l} + b'$. Since $l \leq k$ and $a' h^{l} - a h^k = b - b' \in Ah^{l} \cap B = Bh^{l}$ there exists $d \in B$ such that $b = b' + dh^{l}$, consequently from~\eqref{eq:strictB} we obtain that
$$\textstyle x_\alpha(b/{h^s})\, u = \bigl(x_\alpha({b'}/{h^s})\, u\bigr) \cdot \bigl(u^{-1}\, x_\alpha(dh^{n})\, u\bigr) =  \bigl(x_\alpha({b'}/{h^s})\, u\bigr) \cdot \lambda_h \bigl(\eta_0(x_\alpha(dh^{k - s}))\bigr).
$$
