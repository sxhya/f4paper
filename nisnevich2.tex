...
\cref{lpb} is due to the first-named author whose work was supported by Young Russian Mathematics award, and by the Ministry of Science and Higher Education of the Russian Federation, agreement No. 075-15-2019-1619.
\cref{theorem:LP-for-K2} is due to the second-named author, who was supported by the Russian Science Foundation grant No. 19-71-30002
\cref{glueing} is due to the third-named author, who was supported by the Foundation for the Advancement of Theoretical Physics and Mathematics ``BASIS``.
...
$\lambda_{h}\colon \St(\Phi, A) \to \St(\Phi, A_h)$, $\overline{\iota}\colon\St(\Phi, B_h) \to \St(\Phi, A_h)$. In particular the functors $\St(\Phi, -)$, $\K_2(\Phi, -)$ satisfy weak affine Nisnevich excision for domains and the following sequence of pointed sets is exact in the central term
\begin{equation} \begin{tikzcd} \K_2(\Phi, B) \ar{rrr}{g \mapsto (\lambda_h(g), \iota(g))} & &  & \K_2(\Phi, B_h) \times \K_2(\Phi, A) \ar{rrrr}{(g_1,g_2) \mapsto \overline{\iota}(g_1) \lambda_{\iota(h)}(g_2)^{-1}} & & & & \K_2(\Phi, A_h) \end{tikzcd} \end{equation}
\end{theorem}
...
Our goal is to show that the formula $[u, v] \mapsto \overline{\iota}(u) \cdot \lambda_h^A(v)$ defines a bijection between $V$ and $\St(\Phi, A_h)$. The first step in this direction is to construct an action of $\St(\Phi, A_h)$ on $V$. 
...
Now if \(u \in \St(\Phi, B_h)\) and \(v \in \St(\Phi, A)\) are such that \(\iota(u) = \lambda_h(v)\), the class of \((u, v^{-1})\) in \(V\) maps to \(1 \in \St(\Phi, A_h)\). Thus, \([u, v^{-1}] = [1, 1]\) and $u = \lambda_h(w)$, $v = \iota(w)$ for some $w \in \St(\Phi, B)$, which completes the proof of the assertion of Theorem~\ref{glueing} for Steinberg groups. Since $\Gsc(\Phi, B)\to \Gsc(\Phi, A)$ is injective, we also obtain the assertion for $\K_2(\Phi, -)$.
