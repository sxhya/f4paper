...
Unwinding the definitions, we see that the commutativity of~\eqref{eq:conj-pis} is equivalent to the fact that for every $g\in \St(\Phi, R_S)$ there exists $s_g \in S$ (notice that $s_g = conj(g)^*(1)$) and a homomorphism $c_g := conj(g)_1\colon \St(\Phi, s_g R) \to \St(\Phi, R)$ such that $\lambda_S(c_g(x)) = g\lambda_S(i(x))g^{-1}$ for all $x\in \St(\Phi, s_g R)$, where $i$ denotes the homomorphism $\St(\Phi, s_g R) \to \St(\Phi, R)$ induced by the obvious embedding $s_gR \hookrightarrow R$.
\end{rem}
...
\begin{corollary}
\label{vorcor}
For $f \colon B \to A$ and $h \in B$ as above and every $g \in \St(\Phi, B_h)$ there exists $n = n(g) \in \mathbb{N}$ 
and group homomorphisms $c_g \colon \St(\Phi, h^nB) \to \St(\Phi, B)$, $c_{\overline{f}(g)} \colon \St(\Phi, f(h)^nA) \to \St(\Phi, A)$
such that the following equalities are fulfilled:
\begin{align}
 \label{eq:coherence} f \circ c_g = c_{\overline{f}(g)} \circ f, & \\
 \label{eq:strictB} \lambda_h^0 (c_g (x)) = g \cdot \lambda_h^n(x) \cdot g^{-1} &\text{ for every }x \in \St(\Phi, h^nB), \\
 \label{eq:strictA} \lambda_h^0 (c_{\overline{f}(g)}(y)) = \overline{f}(g) \cdot \lambda_h^n(y) \cdot \overline{f}(g)^{-1} &\text{ for every }y \in \St(\Phi, f(h)^nA).
\end{align}
Here $\lambda_h^k$ (resp. $\lambda^k_{f(h)}$) denotes the homomorphism of Steinberg grops induced by the obvious rng homomorphism $h^kB \to B_h$ (resp. $f(h)^kA \to A_{f(h)}$), where $k \in \mathbb{N}$.
\end{corollary}
\begin{proof}
...
Observe that Theorem's condition $A/hA = B/hB$ is equivalent to the fact that for every $k \geq 0$ one has $A = Ah^k + B$ and $Ah^k \cap B = Bh^k$. Choose any $k \geq n + s$ and decompose $c$ as $c = ah^k + b$ for some $a \in A$, $b \in B$. 
...
The right hand side of~\eqref{eq:generator-action} is independent of either the choice of a decomposition $c = ah^k + b$, the choice of a representative $c_{\overline{\iota}(u^{-1})}$ for the morphism $conj(u^{-1})$ or the choice of a representative \((u, v)\) for $[u, v]$.
\end{lemma}
\begin{proof}
First, let us demonstrate the independence of~\eqref{eq:generator-action} of the choice of a decomposition for $c$. 
...
$$\textstyle x_\alpha(b/{h^s})\, u = x_\alpha({b'}/{h^s})\, u \cdot u^{-1}\, x_\alpha(dh^{n})\, u =  x_\alpha({b'}/{h^s})\, u \cdot \lambda_h \bigl(c_{u^{-1}}(x_\alpha(dh^{l-s}))\bigr)\text{ hence} $$
\begin{align*}
\bigl[x_\alpha(b/{h^s})\, u,\ c_{\overline{\iota}(u^{-1})}(x_\alpha(ah^{k - s}))\, v\bigr] = \bigl[x_\alpha(b'/{h^s})\, u,\ \iota\bigl(c_{u^{-1}}(x_\alpha(dh^{l-s}))\bigr)\, c_{\overline{\iota}(u^{-1})}(x_\alpha(ah^{k - s}))\, v\bigr] & \\
= [x_\alpha(b'/{h^s})\, u,\ c_{\overline{\iota}(u^{-1})}(x_\alpha(ah^{k - s} + dh^{l-s}))\, v\bigr]& \text{by~\eqref{eq:coherence}.}
\end{align*}
Since $ah^k + dh^l = a'h^l$ and $A$ is a domain we obtain that $ah^{k-s} + dh^{l-s} = a'h^{l-s}$, so the above expression coincides with the right-hand side of~\eqref{eq:generator-action} (corresponding to the decomposition $c = a'h^l + b'$).

Next, let us verify the independence of~\eqref{eq:generator-action} of the choice of representatives $c_{u^{-1}}$, $c_{\overline{\iota}(u^{-1})}$. 
Suppose that $c_{u^{-1}}'$ and $c_{\overline{\iota}(u^{-1})}'$ are homomorphisms of index $1$ different representatives for the same pro-group morphisms
 $ $
Let the strict representatives $\eta'$ and $\theta'$ be equivalent to $\eta$ and $\theta$ respectively. By definition of the equivalence, there is $n \geq \max(\theta^*(0) + s, {\theta'}^*(0) + s)$ such that 
$$
